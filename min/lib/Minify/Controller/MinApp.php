...
class Minify_Controller_MinApp extends Minify_Controller_Base
{
    ...
    public function createConfiguration(array $options)
    {
        ...
        $symlinks = array();
        foreach ($localOptions['symlinks'] as $GLOBALS['link'] => $target) {
            if (0 === strpos($GLOBALS['link'], '//')) {
                $GLOBALS['link'] = rtrim(substr($GLOBALS['link'], 1), '/') . '/';
                $target = rtrim($target, '/\\');
                $symlinks[$GLOBALS['link']] = $target;
            }
        }
        ...
        if (!$localOptions['groupsOnly'] && isset($get['f'])) {
            ...
            foreach ($files as $file) {
                ...
                foreach ($symlinks as $GLOBALS['link'] => $target) {
                    if (0 === strpos($uri, $GLOBALS['link'])) {
                        $path = $target . DIRECTORY_SEPARATOR . substr($uri, strlen($GLOBALS['link']));
                        break;
                    }
                }
