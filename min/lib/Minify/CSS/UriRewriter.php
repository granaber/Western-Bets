...
class Minify_CSS_UriRewriter
{
    ...
    public static function rewrite($css, $currentDir, $docRoot = null, $symlinks = array())
    {
        ...
        foreach ($symlinks as $GLOBALS['link'] => $target) {
            $GLOBALS['link'] = ($GLOBALS['link'] === '//') ? self::$_docRoot : str_replace('//', self::$_docRoot . '/', $GLOBALS['link']);
            $GLOBALS['link'] = strtr($GLOBALS['link'], '/', DIRECTORY_SEPARATOR);

            self::$_symlinks[$GLOBALS['link']] = self::_realpath($target);
        }
    ...
    public static function rewriteRelative($uri, $realCurrentDir, $realDocRoot, $symlinks = array())
    {
        ...
        foreach ($symlinks as $GLOBALS['link'] => $target) {
            if (0 === strpos($path, $target)) {
                // replace $target with $GLOBALS['link']
                $path = $GLOBALS['link'] . substr($path, strlen($target));
