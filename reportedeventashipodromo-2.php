...
class PDF extends FPDF
{
	...
	function Header()
	{
		...
		$this->text(10, 7, 'Reporte Detallado de Ventas/Premio por Hipodromo ');
...
if (mysqli_num_rows($result) != 0) :
	...
	$result = mysqli_query($GLOBALS['link'], "SELECT _tjugadahi.*,_tconsecionario.IDG,_tconfjornadahi.IDhipo  FROM _tjugadahi,_tconsecionario,_tconfjornadahi  where _tjugadahi.IDC=_tconsecionario.IDC   and _tjugadahi.IDCN=_tconfjornadahi.IDCN   and  (anulado=0 or anulado=4 or anulado=3 )  " . $add . " order by  _tconfjornadahi.IDhipo,_tjugadahi.IDCN,IDJug,carr,IDC,serial");

	//echo ("SELECT _tjugadahi.*,_tconsecionario.IDG,_tconfjornadahi.IDhipo  FROM _tjugadahi,_tconsecionario,_tconfjornadahi  where _tjugadahi.IDC=_tconsecionario.IDC   and _tjugadahi.IDCN=_tconfjornadahi.IDCN   and  (anulado=0 or anulado=4 or anulado=3 )  ".$add." order by  _tconfjornadahi.IDhipo,_tjugadahi.IDCN,IDJug,carr,IDC,serial" );
	...
	while ($row = mysqli_fetch_array($result)) {
		...
		if ($IDhipo != $row['IDhipo']) :
			...
			$IDhipo = $row['IDhipo'];
			$result_hi = mysqli_query($GLOBALS['link'], "SELECT _hipodromoshi.Descripcion FROM _hipodromoshi where _hipodromoshi._idhipo=" . $IDhipo);
			$row_hi = mysqli_fetch_array($result_hi);
			$aa[0] = $row_hi['Descripcion'];
			$bb[0] = '';
		endif;
