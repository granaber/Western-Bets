...
$IDG=$_REQUEST['IDG']; 
switch($_REQUEST['frma']){

	case 1:
			$formato='_tusu.tipo!=3';
			break;
	case 2:
			$formato='_tusu.tipo=3';
			break;
...
if ($IDG==0):
	...
	$grid->render_sql("SELECT _tusu.* FROM _tusu,_tconsecionario where _tusu.Asociado=_tconsecionario.IDC  and  $formato  ","IDusu" ,"Usuario,Nombre,Tipo,Estatus");
 ...
 /*function my_update($data){
	  ...
	  function formatting($row){
				...
				switch ($row->get_value("Tipo")){
					...
					case 1:$row->set_value("Tipo",'Usuario'); break;
					case 2:$row->set_value("Tipo",'Administrador'); break;
					case 3:$row->set_value("Tipo",'Vendedor'); break;
					case 4:$row->set_value("Tipo",'Agente'); break;
					case 5:$row->set_value("Tipo",'Sistema'); break;
