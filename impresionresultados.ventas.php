...
class PDF extends FPDF
{
	...
	function Header()
	{
		...
		$this->text(10, 7, '..::Betgambler::. Resultado de: ' . $descripcion . '  Fecha:' . Fechareal(-26, "d-m-y"));
	...
	function Header2()
	{
		...
		$this->Cell(10, 7, '..::Betgambler::.  Resultado de: ' . $descripcion . '  Fecha:' . Fechareal(-26, "d-m-y"));
		//$this->Image('logo_pb.png',10,8,33);
		$this->SetFont('Arial', 'B', 7);
		$this->Ln();
	}
...
function listIDG($d)
{
	...
	$listadoIDG = [];
	$cant = 0;
	$resultj = mysqli_query($GLOBALS['link'], "SELECT * FROM _jornadabb where Fecha='" . $d . "'");
	...
	while ($row = mysqli_fetch_array($resultj)) {
		...
		if (mysqli_num_rows($resultj2) != 0) {
			$listadoIDG[] = $row["Grupo"];
		}
	}

	return ["Listado" => $listadoIDG, "cant" => $cant, "IDJ" => $idj];
}
