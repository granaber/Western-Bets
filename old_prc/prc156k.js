...
function captarOdds(idj){
	var larre=new Array(),selcas=new Array();
	...
	if (larre.length!=0){
		for (var i=0;i<=listacs_id.length-1;i++){
			if ($('casino-'+listacs_id[i]).checked)
				selcas.push(listacs_id[i])
		}
		if (selcas.length===0 || selcas.length===listacs_id.length) vselcas=0; else vselcas=selcas.join(',')
...
function onclick_sport(id,parcial){
		if (id!='0' && !parcial)
			$('ckec'+id).checked=!$('ckec'+id).checked;
		else{
			for (var i=0;i<=lista_id.length-1;i++){
				if (parcial){
					var nvl=lista_id[i].split('-');
...
function onclick_casino(id){
	if (id!=0)
		$('casino-'+id).checked=!$('casino-'+id).checked;
	else{
		for (var i=0;i<=listacs_id.length-1;i++){
