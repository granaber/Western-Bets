...
class PDF extends FPDF
{
	...
	function Header()
	{
		...
		$this->text(10, 7, 'Reporte de Ventas por Jugada/Hipodromo ');
...
if (mysqli_num_rows($result) != 0) :
	...
	echo ("SELECT _tjugadahi.*  FROM _tjugadahi  where    (anulado=0 or anulado=4 or anulado=3 )  " . $add . " order by IDC,IDJug,IDCN,carr,serial");
	...
	//// Participacion
	//$aa[$i+3]=number_format( ($diferencia*$datosr[1])/100 , 2, ',', '.').'('.$datosr[1].'%)'; $bb[count($bb)]=0;
	//$aat[count($aat)]+= ($diferencia*$datosr[1])/100; $bbt[count($bbt)]+=0;

	$aa[13] = number_format((($diferencia * $datosr[1]) / 100), 2, ',', '.') . '(' . $datosr[1] . '%)';
	$bb[13] = 0;
