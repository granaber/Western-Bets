...
class Cpdf
{
    ...
    if (!isset($this->fonts[$fontName])) {
      ...
      if (isset($this->fonts[$fontName])) {
        ...
        if (substr($fontName, -4) == '.afm' && strlen($fbtype)) {
          ...
          $tmp = array(
            'BaseFont' => $adobeFontName, 'Widths' => $widthid, 'FirstChar' => $firstChar, 'LastChar' => $lastChar, 'FontDescriptor' => $fontDescriptorId
          );
          if ($fbtype == 'ttf') {
  ...
  function addPngFromFile($file, $x, $y, $w = 0, $h = 0)
  {
    ...
    $options = array(
      'label' => $label, 'data' => $idata, 'bitsPerComponent' => $info['bitDepth'], 'pdata' => $pdata, 'iw' => $info['width'], 'ih' => $info['height'], 'type' => 'png', 'color' => $color, 'ncolor' => $ncolor
    );
