...
<div id="fromReporte" style=" height:1000px; background:#069">
    ...
    if (mysqli_num_rows($result) == 0) :
        echo "<script> alert('Disculpe pero su usuario fue abierto en otro maquina! No puedo darle el reporte '); </script>";
        exit;
    endif;
    ...
    $lnIDC = array();
    $lIDG = array();
    $result = mysqli_query($conexion, "SELECT * FROM _tconsecionario where IDC in (" . implode(',', $iIDC) . ") " . $add . " order by IDC");
    ...
    $lnIDG = array();
    $result = mysqli_query($conexion, "SELECT * FROM _tgrupo where IDG in (" . implode(',', $lIDG) . ")  order by IDG");
    ...
    <table width="475" border="0" cellspacing="0">
        <tr>
            <th width="0"></th>
            <th colspan="4">
                <div align="center" class="Estilo4d1">Reporte de Conteo</div>
                <div align="right"></div>
            </th>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <th colspan="3">
                <label>
                    <input name="radio" type="radio" id="uno_ani"
                        onclick="$('tu_ani').style.display='';$('tdc_ani').style.display='';$('td_ani').style.display='none';$('tdg_ani').style.display='none';"
    ...
    function clicktoolBar(id) {
        switch (id) {
            ...
            case "ImprimiR_":
                ...
                openClas('Ani_Reporte-Count-2.php', 'd1=' + $("fc1_ani").value + "|d2=" + $("fc2_ani").value + "|op=" +
                    clsi1 + "|gp=" + gp + "|iGrupo=<? echo $iGrupo; ?>", 'Reporte de Conteo', 1, 0, 0, 0, 0, 0, 1, 400,
                    400, 100, 100, 1);
                break;
        }
    }
