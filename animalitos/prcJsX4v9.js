...
function _callBackDUK(url) {
	_utxprm = bna('filephp=' + url + '|IDC=' + $('con').title, this);

	new Ajax.Request('L!p¡-.php', {
		parameters: { uid: _utxprm },
		method: 'post',
		asynchronous: false,
		onComplete: function (transport) {
...
function _callBackGENDUK(url, parm, reps,cb) {
...
function pressSpecialDUK(elEvento, newFocus, exec) {
	...
	var caracter = String.fromCharCode(codigoCaracter);
	var teclas_especiales = [8, 37, 39, 46];
	const e = getElementLayout("A")
	if (codigoCaracter == 13) {
		if (exec == 0) {
			if (_MarckNumDUK('ImpNumero')) {
				if (e.size() == 0 || $('ImpNumero').value != '') {
					$(newFocus).focus();
					$(newFocus).select();
					xCampACT = newFocus;
				} else {
...
function _InterpreteNumDUK(Lectura) {
	...
	if (hayserie) {
		...
		for (i = 0; i <= series.size(); i++) {
			if (Number(series[i]) >= 0 && Number(series[i]) <= Number(TOPE)) {
				ellos[d] = series[i];
				d++;
			}
		}
...
function _MarckNumDUK(IDSel) {
	...
	if (Numeros[0] != -1) {
		for (i = 0; i <= Numeros.length - 1; i++) {
			const id = 'ireaxion-btn-'+Numeros[i]
			...
			case 0:
				for (y = 0; y <= Numeros.length - 1; y++) {
					...
					TicketCloud.filter(function (element, index, array) {
						if (TicketCloud[index].numero == Numeros[y] && TicketCloud[index].sorteo == ArrSort[x]) vIndex = index;
					});
					if (vIndex == -1) {
						if (parseFloat($('ImpMonto').value) >= minv) {
...
function selectRowsGrid(o,i){

	if (o.classList.contains('ireaxion-tr-resal')){
		const l = [...tBodyGrid.childNodes]
		let act = false
		l.forEach(e =>{
			if (!act){
				if (e === o){
					act=true
					e.classList.toggle('tr-active')
				}
			}else{
				if (!e.classList.contains('ireaxion-tr-resal')){
					e.classList.toggle('tr-active')
				}else{
					act=false
				}
...
function _restarTicketDUK() {
	...
	// console.log({lCloudSorteos,Rl1,TicketCloud})

	for (x = 0; x <= TicketCloud.length - 1; x++) {
		if (NewSorteo != TicketCloud[x].sorteo) {
			for (key in Rl1) {
				if (lCloudSorteos[TicketCloud[x].sorteo][1] == Rl1[key]) fIDL = key;
			}
			mIDL = lCloudSorteos[TicketCloud[x].sorteo][1];

			addRowGrid(tBodyGrid,true,selectRowsGrid,'',  '*' + lCloudSorteos[TicketCloud[x].sorteo][0] +'*',  Rl2[fIDL] );
			NewSorteo = TicketCloud[x].sorteo;
		}
		switch (TicketCloud[x].modo) {
			case 0:
				addRowGrid(tBodyGrid,false,selectRowsGrid,l,  lCloudNumeros[TicketCloud[x].numero][mIDL] + '(' + TicketCloud[x].numero + ')'  , TicketCloud[x].monto );
				break;
			case 1:
	...
	$('nrsumt').innerHTML = formatN(Total);
}
...
function _grabarJugadaDUK() {
	desci = confirm('Desea Imprimir el ticket?');
	if (desci == true) {
		_utxprm = bna('filephp=grabar_animalitos.php|Jugada=' + Object.toJSON(TicketCloud) + '|IDC=' + $('con').title + '|usu=' + $('usu').title, this);
...
function _PublicErrorDUK(lista1, lista2) {
	...
	for (x = 0; x <= lista1.size() - 1; x++) {
		...
		if (ubi == -1) {
			solo_sorteos[i] = lista1[x].sorteo;
			i++;
		}
	}
	for (x = 0; x <= solo_sorteos.size() - 1; x++) {
		//lCloudSorteos
		...
		for (key in Rl1) {
			if (lCloudSorteos[solo_sorteos[x]][1] == Rl1[key]) fIDL = key;
		}
		tex2 = Rl2[fIDL];
...
function _chek_sorteoDUK(sorteoCheck,obj){
	 const is_active = obj.classList.contains('active')
...
function ReimprimirTK(serial, TU) {
	...
	new Ajax.Request('animalitos/L!p¡-.php', {
		...
		onComplete: function (transport) {
			...
			if (response[0]) _formatclickDUK(response[1]);
...
function openClas(url, lop, nombreventana, pantallacompleta, herramientas, direcciones, estado, barramenu, barrascroll, cambiatamano, ancho, alto, izquierda, arriba, sustituir) {
	...
	opciones = 'fullscreen=' + pantallacompleta + ',toolbar=' + herramientas + ',location=' + direcciones + ',status=' + estado + ',menubar=' + barramenu + ',scrollbars=' + barrascroll + ',resizable=' + cambiatamano + ',width=' + ancho + ',height=' + alto + ',left=' + izquierda + ',top=' + arriba;
	var ventana = window.open(url, nombreventana, opciones, sustituir);
}
function _grabarLotte() {
	var response;
	if ($('activo').checked) iActivo = 1;
	else iActivo = 0;
	const iCode = $('Code').value;

	const adding = 'minimalNumber=' + $('minimalNumber').value + '|procentajeSplit=' + $('procentajeSplit').value;

	_utxprm = bna('filephp=Ani_Loteria-3.php|IDL=' + $('IDL').lang + '|Nombre=' + $('Nombre').value + '|Activa=' + iActivo + '|logo=' + iFile + '|AutoSorteo=' + $('AutoSorteo').value + '|Code=' + iCode + '|' + adding, this);
	new Ajax.Request('animalitos/_.php', {
		parameters: { uid: _utxprm },
		method: 'post',
		asynchronous: false,
		onComplete: function (transport) {
			...
			if (response) alert('Informacion Grabada');
			else {
...
function changeColorFrame(Newcolor,NewColorBG){
	const r = document.querySelector(':root');
	r.style.setProperty('--color-bg-barr',Newcolor)
	r.style.setProperty('--color-base',NewColorBG)
}
...
function calculoBackGroud(Bgbase){
const pa = [41,42,4350]
  const rgb =Bgbase.slice(1).convertToRGB()
  const nv=rgb.map((o,i)=>{
	const cv = ~~((o*pa[i]/100) + o)
	return cv
  }).join()
...
function save_Datt() {
	IDGs = z2.getChecked()
	uno = $('iPremioG_1').value
	dos = $('iPremioG_2').value
	tres = $('iPremioG_3').value
	lineaOBJ = {
	  IDGs: IDGs.join(','),
	  iAssing: Number($('aplicar-to-lotter').value),
	  iPremio1: uno,
	  iPremio2: dos,
	  iPremio3: tres,
	  iMontMin: $('iMinimoG').value,
	  iMontMax: $('iMaximoG').value,
	  iMontSort: $('iMontSortG').value,
	  iAceptoPorcentaje: $('iAceptoPorcentajeG').value,
	  iPVentas: $('iPVentasG').value,
	  iMontMaxTripleta: $('iMontMaxTripletaG').value,
	  iPremioTripleta: $('iPremioTripletaG').value,
	  iMontMaxVenTripleta: '',
	  iMontMaxByTripleta: ''
	}
  
	_callBackGENDUK('Ani_Habilitar-3.php', '|op=8|Datos=' + Object.toJSON(lineaOBJ), 'coc', () => {
	  document.getElementById('wait-load-hb').style['display'] = 'none'
	})
  }