...
if (mysqli_num_rows($result) != 0) :
  ...
  foreach ($arraF as $idj => $value) {
    ...
    while ($row2n1 = mysqli_fetch_array($result2n1)) {
      ...
      echo "<br><h2>" . $row2n1['Nombre'] . "</h2>";
      echo '<table  border="1" cellspacing="0" cellpadding="2"><tr>';
      echo '<td  width="120" align="center" style="border-bottom:none;border-right:thin; border-left:thin; border-top:thin; background:white">Premios</td>';
      $IDL = $row2n1['IDL'];
      $aID = array();
      ...
      while ($row = mysqli_fetch_array($result)) {
        ...
        $myWin1 = getNumber($row['G1'], $IDL);
        $myWin2 = getNumber($row['G2'], $IDL);
        $myWin3 = getNumber($row['G3'], $IDL);

        if ($show_premio) {
          echo '<td align="center" style="border-bottom:none; border-top:none; border-right:thin; border-left:thin; background:white"><section>';
          if ($myWin1[0] != '-1') :
            echo '<h3>1<sup>ro</sup></h3>';
          endif;
          if ($myWin2[0] != '-1') :
            echo '<h3>2<sup>do</sup></h3>';
          endif;
          if ($myWin3[0] != '-1') :
            echo '<h3>3<sup>ra</sup></h3>';
          endif;
        ...
        echo '<td align="center" style="border-bottom:none; border-top:none; border-right:thin; border-left:thin; background:' . $color . '"><section>';
        if ($myWin1[0] != '-1') :
          echo '<h2>' .  $myWin1[1] . '</h2>';
        endif;
        if ($myWin2[0] != '-1') :
      ...
      //       break;
      //   }
      //   echo ' <td align="center" style="border-right:thin; border-left:thin; border-bottom:thin;border-top:none; background:' . $color . '"><h3>' . $EnLetrayNum[$i] . '</h3></td>';
      // }
      // echo '</tr>';
