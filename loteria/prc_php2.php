...
$server = 'zuloteria.db.5144062.hostedresource.com'; //zuloteria.db.5144062.hostedresource.com;localhost
$user = 'zuloteria'; //root;zuloteria
$clv = 'Legna113'; //Legna113;intra
$db = 'zuloteria'; //wmlottery;zuloteria
...
function ProcesoEscrute($IDJ, $IDLot, $Numero, $Adicional)
{
	...
	while ($rowprint = mysqli_fetch_array($resultprint)) {
		...
		if ($rowprint['activo'] == 1) :
			if ($IDC != $rowprint['IDC']) :
				$IDC = $rowprint['IDC'];
				$PagoPremioTotal = PagoporConcesion($IDC);
...
function PagoporConcesion($IDC)
{
	$pagopremio = array();
	$Formuladepago = -1;
	$Formuladepremio = -1;

	$result2 = mysqli_query($GLOBALS['link'], "Select * from _tcupos where Tipo=4 and ID_Relacionado=" . $IDC);
	...
	if (mysqli_num_rows($result2) == 0) :
		...
		for ($i = 0; $i <= count($relacion) - 1; $i++) {
			...
			if ($Arelacion[0] != '') :
				$result2 = mysqli_query($GLOBALS['link'], "Select * from _tcupos where Tipo=" . $Arelacion[0] . " and ID_Relacionado=" . $Arelacion[1]);
				echo ("Select * from _tcupos where Tipo=" . $Arelacion[0] . " and ID_Relacionado=" . $Arelacion[1]);
				if (mysqli_num_rows($result2) != 0) :
...
function relacionaNivel($Kserial, $KNoOptar)
{
	...
	//echo ("Select * from _trelacionbanca where ".$campoR[intval($Relacion[0])-1]."=".$Relacion[1]);
...
function CuposMaximo($Numero, $IDLot, $IDC, $IDJ, $Monto, $idx)
{
	...
	$triple = 0;
	$terminal = 0;
	$result = mysqli_query($GLOBALS['link'], "SELECT * FROM _tcupos  where ID_Relacionado=" . $IDC . " and Tipo=4");
	if (mysqli_num_rows($result) != 0) :
		$row = mysqli_fetch_array($result);
	...
	for ($j = 0; $j <= count($relacion) - 1; $j++) {
		...
		$result = mysqli_query($GLOBALS['link'], "SELECT * FROM _tcupos  where ID_Relacionado=" . $AKRla[1] . " and Tipo=" . $AKRla[0]);
		if (mysqli_num_rows($result) != 0) :
			$row = mysqli_fetch_array($result);
			...
			switch (count($NumeroS)) {
				case 2:
					if ($terminal != 0) :
						...
						$Resultado = $row['LimiteTerminal'] - $MontoVendido;
					endif;
					break;
				case 3:
					if ($triple != 0) :
						...
						$Resultado = $row['LimiteTriple'] - $MontoVendido;
					endif;
					break;
			}
