...
switch(id){
	case "incluir_":
					 makeResultwin("zona-1.php?op=1","gridbox");
					break;
	case "modificar_":
	                 makeResultwin("zona-1.php?op=2&IDZ="+$('idseleccionado').lang,"gridbox");
					break;
	case "Cerrar_":
 ...
 function doOnRowSelected(id){ 
   ...
   function doOnCheck(rowId,cellInd,state){
	...
	if (state)
	  ...
	  estado=0;
	makeResultwin("chaceStatus.php?SqlStatus=Update _tzona set Estatus="+estado+" where IDZ="+rowId,"gridbox");
	} 
  	var dhxWins1 = new dhtmlXWindows();
	
    dhxWins1.setImagePath("codebase/imgs/");	
	var w1 = dhxWins1.createWindow("w1",10, 80, 540, 200);
	w1.setText("Configuracion de Zonas");
	w1.attachObject('obj');
	...
	createByxml('grid_Zona.xml','SELECT * FROM _tzona Order by IDZ','IDZ|Descripcion|Responsable|Estatus');
	mygrid = w1.attachGrid();
	mygrid.setImagePath("codebase/imgs/");
	mygrid.setHeader("ID,Descripcion,Responsable,Status");
