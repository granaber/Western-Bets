...
while ($row = mysqli_fetch_array($result)) {
	...
	for ($u = 0; $u <= count($jgdad) - 2; $u++) {
		...
		if ($addop == 'o') :
			$valores = posicionOpcion1($idj, $v_d_escrute[2], $equi, $logro);
			$equi = $valores[0];
		endif;


		$result2 = mysqli_query($GLOBALS['link'], "Select * From _partidosbb Where (IDE1=" . $equi . " or IDE2=" . $equi . ") and IDJ=" . $idj . " and Grupo=" . $v_d_escrute[2]);
		$row2 = mysqli_fetch_array($result2);

		if (diferenciadehoras($fc, $row2['Hora'], $hora)) :
			echo $row['serial'] . ' Hora ticket:' . $hora . ' Hora del partido: ' . $row2['Hora'] . ' Letra: ' . $row['IDC'] . '<br>';

			// $result7 = mysqli_query($GLOBALS['link'],"Update _tjugadabb set activo=2,hc='Por fuera de juego' Where serial=".$row['serial']); 	
			break;
		/*else:
