...
switch(id){
	case "incluir_":
					 makeResultwin("agencia-1.php?op=1","gridbox");
					break;
	case "modificar_":
	                 makeResultwin("agencia-1.php?op=2&IDCtr="+$('idseleccionado').lang,"gridbox");
					break;
	case "Cerrar_":
 ...
 function doOnRowSelected(id){ 
   ...
   function doOnCheck(rowId,cellInd,state){
	...
	if (state)
	  ...
	  estado=0;
	makeResultwin("chaceStatus.php?SqlStatus=Update _tagencias set Estatus="+estado+" where IDC="+rowId,"gridbox");
	} 
  	var dhxWins1 = new dhtmlXWindows();
	
    dhxWins1.setImagePath("codebase/imgs/");	
	var w1 = dhxWins1.createWindow("w1",10, 80, 610, 200);
	w1.setText("Configuracion de Agencia");
	w1.attachObject('obj');
	...
	mygrid.setHeader("ID,Descripcion,Responsable,Telefono,Estatus");
	mygrid.setInitWidths("50,200,150,100,80")
