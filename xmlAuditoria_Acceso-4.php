<br />
<div align="center"><span style="color:#FC0; font-size:16px">** Auditoria de Logros **</span></div>
<br />
...
<script>
	...
	function doOnRowSelected(rowId, cellIndex) {
		...
		if (style != '') {
			...
			makeResultwin2("xmlAuditoria_logros-4.php?Grupo=" + valorescodeGrupo[idselect] + "&IDPr=" + IDPr + "&IDJ=<? echo $IDJ; ?>&IDB=" + IDB + "&IDP=" + IDP + "&TCol=" + textoCol + '&IDDD=' + IDDDjs[idselect][idx], 'fromChangeOdds'); //
		}
		return true;
	}
	...
	for (i = 0; i <= valorescodeGrupo.length - 1; i++) {
		...
		mygridA[i].init();
		mygridA[i].clearAll();
		mygridA[i].loadXML("xmlAuditoria_logros-3.php?Grupo=" + valorescodeGrupo[i] + "&IDB=" + IDB + "&IDJ=" + IDJ);
