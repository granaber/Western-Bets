...
/*$iSecury=explode('/',getenv("HTTP_REFERER"));
	if ( !($iSecury[count($iSecury)-2]==="parlayenlinea.org" || $iSecury[count($iSecury)-2]==="parlayenlinea.net"  || $iSecury[count($iSecury)-2]==="www.parlayenlinea.net"  || $iSecury[count($iSecury)-2]==="www.parlayenlinea.org"  ) ):
...
function resstruc_tick($serial)
{
	...
	for ($u = 0; $u <= count($jgdad) - 2; $u++) {
		...
		$Lineaticket[$u + 1] = $u . '|' . convertirhora($row2['Hora']) . '|' . $code . '-' . $row1['Descripcion'] . ' ' . convertirtk($carr, true) . ' ' . $valoaad . '|' . $logro; //convertirtk($logro,false);
