...
if ($tipo == 2) :
	...
	$org = $_REQUEST['org'];
	$nom = $_REQUEST['nom'];
	$icarr = $_REQUEST['inicarr'];

	if ($icarr == 0) : $icarr = $carr;
	endif;
	...
	if ($fmr == 5) :
		...
		if ($carr == 0) :
			$result = mysqli_query($GLOBALS['link'], "SELECT * FROM _cierrehi where IDJug=" . $IDJugada . " and IDCN=" . $IDCN);
		else :
			$result = mysqli_query($GLOBALS['link'], "SELECT * FROM _cierrehi where IDJug=" . $IDJugada . " and IDCN=" . $IDCN . " and ct=" . $carr);
		endif;
	endif;

	if (mysqli_num_rows($result) == 0) :

		if (verificarcierre($IDCN, $icarr)) :

			$activar = 1;
			$result = mysqli_query($GLOBALS['link'], "SELECT * FROM _tdjuegoshi where IDJug=" . $IDJugada);
			...
			$result = mysqli_query($GLOBALS['link'], "SELECT * FROM _hipodromoshi where _idhipo=" . $row5['IDhipo']);
			$row5 = mysqli_fetch_array($result);
			$hip = $row5['Descripcion'];
	...
	$Serial = $_REQUEST['Serial'];
	$result = mysqli_query($GLOBALS['link'], "SELECT _tjugadahi.*,_tconsecionario.Nombre,_tconsecionario.Direccion FROM _tjugadahi,_tconsecionario where _tjugadahi.IDC=_tconsecionario.IDC and Serial=" . $Serial . " and anulado=0");

	$row = mysqli_fetch_array($result);

	if (mysqli_num_rows($result) != 0) :

		/*$result3 = mysqli_query($GLOBALS['link'],"SELECT * FROM _cierrehi where IDJug=".$row['IDJug']." and IDCN=".$row['IDCN'] ); 
 
 if (mysqli_num_rows($result3)==0):*/
		$result2 = mysqli_query($GLOBALS['link'], "SELECT * FROM _tconfjornadahi where IDCN=" . $row['IDCN']);
		...
		if ($row2['Estatus'] == 1) :
			...
			$fmr = $row['Formato'];
			$k_jug = explode("|", $jugada);
			$result = mysqli_query($GLOBALS['link'], "SELECT * FROM _hipodromoshi where _idhipo=" . $row2['IDhipo']);
			$row5 = mysqli_fetch_array($result);
			$hip = $row5['Descripcion'];
...
<table border="0">
 <tr>
    <th colspan="' . $tdcl . '" scope="col" ><p aling="center">PAGAVEN</p></th>
  </tr>
  <tr>
    <th colspan="' . $tdcl . '" scope="col" ><p aling="center">' . $title . ' HIPISMO INTERNACIONAL</p></th>
  </tr>
   <tr>
    <th colspan="' . $tdcl . '" scope="col" ><p aling="center">Hipodromo: ' . $hip . '</p></th>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <th  valign="top"  bgcolor="#CCCCCC" scope="col" class="alinc" >Nombre</th>';
		$enviar['tk'] = $enviar['tk'] . '<th   valign="top" bgcolor="#CCCCCC" scope="col" class="alinc" >Logro</th><th valign="top" bgcolor="#CCCCCC" scope="col" class="alinc" >Apuesta</th>';
	endif;
	...
	for ($i = 1; $i <= count($k_jug) - 1; $i++) {
		...
		if ($fmr != 5) :
			...
			$enviar['tk'] = $enviar['tk'] . " <th valign='top' scope='col' >" . $rowtablas['Nombre'] . "</th>";
			$enviar['tk'] = $enviar['tk'] . " <th valign='top' scope='col' class='alinr'>" . $valortbl[1] . "</th>";
			$enviar['tk'] = $enviar['tk'] . " <th valign='top' scope='col' class='alinr'>" . $valortbl[0] . "</th>";
		endif;
		$enviar['tk'] = $enviar['tk'] . "</tr>";
	}
	...
	if ($fmr != 5) :
		if ($fmr != 4) :
			$enviar['tk'] = $enviar['tk'] . '<th colspan="2" align="right" valign="top" scope="col">Total:Bsf.</th> <th height="23" scope="col" class="alinr" >' . $formattedR . '</th>';
		else :
		...
		$enviar['tk'] = $enviar['tk'] . '<th colspan="3" align="right" valign="top" scope="col">Total:Bsf.</th> <th scope="col" class="alinr" >' . $formattedR . '</th></tr>';
	endif;
	if ($tipo == 1) :
		$enviar['tk'] = $enviar['tk'] . '<tr><th colspan="2" align="right" valign="top" scope="col">Dividendo:Bsf.</th> <th height="23" scope="col" class="alinr" >' . $_REQUEST['dividendo'] . '</th></tr>' . '<tr><th colspan="2" align="right" valign="top" scope="col">Premio a Pagar:Bsf.</th> <th height="23" scope="col" class="alinr" >' . $_REQUEST['premio'] . '</th></tr>';
	endif;
	if ($tipo == 3) :
		$enviar['tk'] = $enviar['tk'] . '<tr><th colspan="2" align="right" valign="top" scope="col">NO VALIDO</th> <th height="23" scope="col" class="alinr" >NO VALIDO</th></tr>' . '<tr><th colspan="2" align="right" valign="top" scope="col">NO VALIDO</th> <th height="23" scope="col" class="alinr" >NO VALIDO</th></tr>';
	endif;
	if ($tipo != 3) :
		$enviar['tk'] = $enviar['tk'] . '<tr>-  <th>S.E.:' . trim($se) . '</th> <th></th> </tr>';
	else :
		$enviar['tk'] = $enviar['tk'] . '<tr>-  <th>COPIA-COPIA-COPIA</th> <th></th> </tr>';
	endif;
