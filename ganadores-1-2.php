...
if ($apli == 1) :
  ...
  for ($i = 0; $i <= count($lista) - 1; $i++) {
    $result = mysqli_query($GLOBALS['link'], "Select * from _ganadores where idcn=" . $idcn . " and carr=" . ($i + 1));
    if (mysqli_num_rows($result) == 0) :
      $result = mysqli_query($GLOBALS['link'], "INSERT INTO _ganadores (idcn,ganadores,carr,cirr) VALUES (" . $idcn . ",'" . $lista[$i] . "'," . ($i + 1) . ",'1')");
    // echo "INSERT INTO _ganadores (idcn,ganadores,carr,cirr) VALUES (".$idcn.",'".$lista[$i]."',".($i+1).",".$lc[$i].")"; //(carr... values ($idcn,$i+1,$lista[$i],$lc[0])
    else :
      $result = mysqli_query($GLOBALS['link'], "Update _ganadores Set ganadores='" . $lista[$i] . "',cirr='1' Where idcn=" . $idcn . " and carr=" . ($i + 1));
