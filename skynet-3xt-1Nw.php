...
$result2x = mysqli_query($GLOBALS['link'], "Select * From _tbjuegodd where Grupo=" . $Grupo . " Order by Formato,IDDD ",);
while ($row2x = mysqli_fetch_array($result2x)) {
	...
	$IDDD = $row2x['IDDD'];
	$result = mysqli_query($GLOBALS['link'], "Select * From _tblogrosXmlNT where IDDD=" . $IDDD);
	...
	if (mysqli_num_rows($result) != 0) :
		...
		$lcasino = explode(',', $casino[$i]);
		$acasino = array();
		for ($x = 0; $x <= count($lcasino); $x++)
		...
		$Plogros1 = 0;
		$Plogros2 = 0;
		$endpoint = "http://superpoolhipico.com:8910/serviceV2";
		$query = <<<'GRAPHQL'
query tblogrosNT($idequi:Int!,$idj:Int!,$periodo:Int!,$tp:[Int]){
	tblogrosNT(idequi:$idequi,idj:$idj,periodo:$periodo,tp:$tp){
		linea
		idequi
		periodo
		logro
...
GRAPHQL;
		// print_r(['idequi' => $aCode1[$j],'idj'=>$aidj[$i],'periodo'=>$periodo2,'tp'=>$acasino]);
		switch ($periodo) {
			case 1000:
				$result = mysqli_query($GLOBALS['link'], "Select * From _tblogrosXmlNT where IDDD=" . $proceso);
				$row = mysqli_fetch_array($result);
				...
				// $result = mysqli_query("Select * From _tblogrosNT where idequi=".$aCode1[$j]." and idj=".$aidj[$i]." and periodo=".$periodo2.$add ,$skynet);
				// $result=graphqlQuery($endpoint, $query, ['idequi' => $aCode1[$j],'idj'=>$aidj[$i],'periodo'=>$periodo2,'tp'=>$acasino]);
				$result = graphqlQuery($endpoint, $query, ['idequi' => intval($aCode1[$j]), 'idj' => intval($aidj[$i]), 'periodo' => intval($periodo2), 'tp' => $acasino]);
			...
			case 1001:
				...
				break;
			default:
				// $result = mysqli_query("Select * From _tblogrosNT where idequi=".$aCode1[$j]." and idj=".$aidj[$i]." and periodo=".$periodo.$add ,$skynet);
				// print_r(['idequi' => intval($aCode1[$j]),'idj'=>intval($aidj[$i]),'periodo'=>intval($periodo),'tp'=>split(',',$casino[$i])]);
				$result = graphqlQuery($endpoint, $query, ['idequi' => intval($aCode1[$j]), 'idj' => intval($aidj[$i]), 'periodo' => intval($periodo), 'tp' => $acasino]);
		...
		switch ($periodo) {
			case 1000:
				//$result = mysqli_query("Select * From _tblogrosXmlNT where IDDD=".$proceso ,$GLOBALS['link']);$row = mysqli_fetch_array($result);
				//$periodo2=$row['periodo'];
				//$proceso=$dproceso;
				if ($_2periodo != -1) :
					$basesigno2 = _findsigno($_2periodo, $aCode2[$j], $aidj[$i], $add);
				endif;
				// $result = mysqli_query("Select * From _tblogrosNT where idequi=".$aCode2[$j]." and idj=".$aidj[$i]." and periodo=".$periodo2.$add ,$skynet);
				$result = graphqlQuery($endpoint, $query, ['idequi' => intval($aCode2[$j]), 'idj' => intval($aidj[$i]), 'periodo' => intval($periodo2), 'tp' => $acasino]);
			...
			case 1001:
				...
				// $result = mysqli_query("Select * From _tblogrosNT where idequi=".$aCode2[$j]." and idj=".$aidj[$i]." and periodo=".$periodo2.$add ,$skynet);
				// $result=graphqlQuery($endpoint, $query, ['idequi' => $aCode2[$j],'idj'=>$aidj[$i],'periodo'=>$periodo2,'tp'=>$acasino]);
				$result = graphqlQuery($endpoint, $query, ['idequi' => intval($aCode2[$j]), 'idj' => intval($aidj[$i]), 'periodo' => intval($periodo2), 'tp' => $acasino]);
				...
				break;
			default:
				// $result = mysqli_query("Select * From _tblogrosNT where idequi=".$aCode2[$j]." and idj=".$aidj[$i]." and periodo=".$periodo.$add ,$skynet);
				$result = graphqlQuery($endpoint, $query, ['idequi' => intval($aCode2[$j]), 'idj' => intval($aidj[$i]), 'periodo' => intval($periodo), 'tp' => $acasino]);
		...
		$result = mysqli_query($GLOBALS['link'], "Select * From _tbjuegodd where IDDD=$IDDD");
		$row = mysqli_fetch_array($result);
		...
		switch ($proceso) {
			...
			case 9:
				...
				if ($Plogros1 != 0) :
					...
					$result = mysqli_query($GLOBALS['link'], "SELECT *  FROM _DBconver  where BaseM=$logro1");
					$row = mysqli_fetch_array($result);
					...
					if ($Plogros2 != 0) :
						...
						$result = mysqli_query($GLOBALS['link'], "SELECT *  FROM _DBconver  where BaseM=$logro2");
						$row = mysqli_fetch_array($result);
			...
			case 8:
				...
				switch ($periodo) {
					case 1001:
						...
						$result = mysqli_query($GLOBALS['link'], "SELECT *  FROM _DBconverC  where Base=" . $nc);
						$row = mysqli_fetch_array($result);
						...
						if ($Plogros2 == 0 && $Plogros1 == 0) :
							...
							if ($PCarrera1 == 0) :
								...
								if ($Sou[0] > $Sou[1]) :
									...
									$resultdbo = mysqli_query($GLOBALS['link'], "SELECT *  FROM _DBconver  where BaseM=$logro2");
