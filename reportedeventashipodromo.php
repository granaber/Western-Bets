...
class PDF extends FPDF
{
	...
	function Header()
	{
		...
		$this->text(10, 7, 'Reporte Detallado de Ventas/Premio (DETALLE CONCESIONARIO)');
...
if (mysqli_num_rows($result) != 0) :
	...
	if ($hastaIDCN == 0) : $hastaIDCN = $desdeIDCN;
	endif;

	$add = " and idcn BETWEEN " . $desdeIDCN . " and " . $hastaIDCN;
	...
	$result = mysqli_query($GLOBALS['link'], "SELECT _tjugadahi.*,_tconsecionario.IDG  FROM _tjugadahi,_tconsecionario  where _tjugadahi.IDC=_tconsecionario.IDC    and  _tconsecionario.estatus=1 and  anulado=0  " . $add . " order by IDJug,IDCN,carr,IDC,serial");

	echo ("SELECT _tjugadahi.*,_tconsecionario.IDG  FROM _tjugadahi,_tconsecionario  where _tjugadahi.IDC=_tconsecionario.IDC    and  _tconsecionario.estatus=1 and  anulado=0  " . $add . " order by IDJug,IDCN,carr,IDC,serial");
...
function montojugadapremio($jugada, $serial)
{
	$Jugada_x_Carrera = explode('|', $jugada);
	$sumaventa = array(0, 0, 0, 0, 0, 0);
	for ($i = 1; $i <= count($Jugada_x_Carrera) - 1; $i++) {
		$ejemplares = explode('-', $Jugada_x_Carrera[$i]);
		$sumaventa[0] += $ejemplares[0];
		$sumaventa[1] += $ejemplares[1];
		$sumaventa[2] += $ejemplares[2];
	}

	$premacion = EscrutarHI($serial, 0);
	$sumaventa[3] += $premacion[0];
	$sumaventa[4] += $premacion[1];
	$sumaventa[5] += $premacion[2];
