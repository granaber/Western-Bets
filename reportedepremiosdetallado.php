...
class PDF extends FPDF
{
	...
	function Header()
	{
		...
		$this->text(10, 7, 'Reporte Detallado de Premios');
...
while ($row = mysqli_fetch_array($result)) {
	...
	if ($atp == $totaldeb) :
		...
		$aa[$uu + 4] = number_format($row['Valor_J'] * $divi, 2, ',', '.'); //calculodepago($row['Valor_J'],$row['Valor_R'],$arrayes[0],$arrayes[1],$parti,$divi,$factor)
		...
		if (strcmp($cabz, $row['IDC']) == 0) :
			...
			$sumxletraByPremio += $aa[$uu + 4];
			$sumxGrupo += $row['Valor_J'];
			$sumxGrupoByPremio += $aa[$uu + 4];
			$pdf->Ln();
		else :
			$_xp = $pdf->Gety();
			$pdf->Sety($_xp + 3);
			...
			$pdf->Setx($xpp - 20);
			$pdf->MultiCell(100, 3, 'Total Premio(' . $cabz . ') : ' . number_format($sumxletraByPremio, 2, ',', '.'));
			if ($grupo != $row['IDG']) :
				$pdf->Ln();
				$pdf->Setx($xpp - 20);
				$pdf->MultiCell(100, 3, 'Total Ventas Grupo(' . $grupo . ') : ' . number_format($sumxGrupo, 2, ',', '.'));
				$pdf->Setx($xpp - 20);
				$pdf->MultiCell(100, 3, 'Total Ventas Grupo(' . $grupo . ') : ' . number_format($sumxGrupoByPremio, 2, ',', '.'));
				$sumxGrupo = 0;
				$grupo = $IDG;
			endif;
			$cabz = $row['IDC'];
