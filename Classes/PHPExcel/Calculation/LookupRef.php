...
class PHPExcel_Calculation_LookupRef {
	 ...
	 * @param	string	$GLOBALS['link']URL		Value to check, is also the value returned when no error
	 * @param	string	$displayName	Value to return when testValue is an error condition
	 * @return	mixed	The value of $displayName (or $GLOBALS['link']URL if $displayName was blank)
	 */
	public static function HYPERLINK($GLOBALS['link']URL = '', $displayName = null, PHPExcel_Cell $pCell = null) {
		$args = func_get_args();
		$pCell = array_pop($args);

		$GLOBALS['link']URL		= (is_null($GLOBALS['link']URL))		? '' :	PHPExcel_Calculation_Functions::flattenSingleValue($GLOBALS['link']URL);
		$displayName	= (is_null($displayName))	? '' :	PHPExcel_Calculation_Functions::flattenSingleValue($displayName);

		if ((!is_object($pCell)) || (trim($GLOBALS['link']URL) == '')) {
			return PHPExcel_Calculation_Functions::REF();
		}

		if ((is_object($displayName)) || trim($displayName) == '') {
			$displayName = $GLOBALS['link']URL;
		}

		$pCell->getHyperlink()->setUrl($GLOBALS['link']URL);
