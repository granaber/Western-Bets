				...
				if ($row['Estatus'] == 1) :
					...
					if ($row["Tipo"] == 3) :
						if (VerificacionHORARIO()) :
							...
							$respuesta = "true||" . $row["Asociado"] . "||" . $row["Estacion"] . "||" . $row["Descripcion"] . "||" . date("d/n/Y") . "||" . $jor . "||" . $rnd . "||" . $row["IDusu"] . "||" . $row["Acceso"] . "||" . $row["AccesoP"] . "||" . $row["bloqueado"] . "||" . $eminutos . "||" . trim($usu);
							$respuesta = "true||&" . base64_encode($respuesta);
						...
						Logs($row['IDusu'], 0, 'Acceso al Sistema', 1);
						$respuesta = "true||" . $valor . "||" . $row["Estacion"] . "||" . $row["Descripcion"] . "||" . date("d/n/Y") . "||" . $jor . "||" . $rnd . "||" . $row["IDusu"] . "||" . $row["Acceso"] . "||" . $row["AccesoP"] . "||" . $row["bloqueado"] . "||" . $eminutos . "||" . trim($usu);
						$respuesta = "true||&" . base64_encode($respuesta);
					endif;
				else :
...
if (isset($row['IDusu'])) {
	$sql = ("update _tusu set hashtime =UNIX_TIMESTAMP(NOW()),lastactivity =UNIX_TIMESTAMP(NOW())  where IDusu = " . $row['IDusu']);
	$queryUP = mysqli_query($GLOBALS['link'], $sql);
}
echo $respuesta;
