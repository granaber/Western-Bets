...
class PDF extends FPDF
{
	...
	function Header()
	{
		...
		$this->text(10, 7, 'Reporte Resumido de Ventas/Premio/Diferencia por Grupos');
...
if (true) :
	...
	$totalgpremios = 0;
	$clventas = 0;
	$result = mysqli_query($GLOBALS['link'], "SELECT * FROM _tconsecionario " . $add1 . " Order By IDG");
	while ($row = mysqli_fetch_array($result)) {
		if ($totalgventas != 0 && $primergrupo != $row['IDG']) :
			$aa = array();
			$resultgrup = mysqli_query($GLOBALS['link'], "SELECT * FROM _tgrupo where IDG=" . $row['IDG']);
			$rowgrup = mysqli_fetch_array($resultgrup);
			$aa[0] = $rowgrup['Descrip'];
