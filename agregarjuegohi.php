<div id="box12" style=" background:#A78D81">
  <?php
...
fc >= 1 ... Busca el registo para modificar o eliminar si existe */
  ...
  if ($pfc == 0) :
    ...
    $result = mysqli_query($GLOBALS['link'], "SELECT * FROM _tdjuegoshi Order by IDJug DESC ");
    ...
    $result = mysqli_query($GLOBALS['link'], "SELECT * FROM _tdjuegoshi where IDJug=" . $pfc);
    $row = mysqli_fetch_array($result);
    $numero = $row["IDJug"];
    $jn = $row["Descrip"];
    $fc = $row["Multip"];
      ...
      <tr>
        <td>
          <table>
            <tbody>
              <tr align="center">
                <td><span style="color: #000000; font-size:16px" align="center"><strong>AGREGAR JUEGO</strong></span></td>
              </tr>
            </tbody>
          </table>
          ...
          <table border="0" cellpadding="2" cellspacing="1" width="100%">
            ...
            <tbody>
              ...
              <tr>
                <td><span style="color:#FFFFFF"><strong>Posición Relacion </strong></span></td>
                <td><select name="select2" size="1" id="relpos">
                    <option selected='selected' value="0">0</option>
                    <?php
                    $result2 = mysqli_query($GLOBALS['link'], "SELECT * FROM _tdjuegoshi order by relpos ASC");
                    $i = 1;
                    ...
                    $result3 = mysqli_query($GLOBALS['link'], "SELECT * FROM _tdjuegoshi order by IDJug");
                    $rp[$i] = 0;
                    $x = 1;
                    $j = 1;
                    $n = 1;
                    ...
                    $result4 = mysqli_query($GLOBALS['link'], "SELECT * FROM _tdjuegoshi order by IDJug");
                    $t = $n;
                    $n = 1;
                    $i = 1;
                    $x = 0;
          ...
          <p>
            <?php
            if (!$pfc == 0) :
              echo  "<input id='btnguardar'  value='Modificar' type='submit'  onclick='grabar_datosJuegohi();'/>";
              echo  "<input id='btneliminar'  value='Eliminar Juego' type='submit'  onclick='elimnar_datosJuegohi();'/>";
            else :
              echo  "<input id='btnguardar'  value='Guardar Nuevo Juego' type='submit'  disabled='disabled'  onclick='grabar_datosJuegohi();'/>";
            endif;
            ?>
            <input name="submit_regresar" value="<-Volver" title="<?php echo $pfc; ?> " onClick="javascript:makeRequest('agregarjuego-1-1hi.php');" type="button" />
          </p>
        </td>
      </tr>
      <tr>
...
<div id="box15" style="background:#666666">
  <table width="536" border="0" cellspacing="0" cellpadding="0">
    ...
    <tr>
      ...
      <td>
        <div id="box13" style="width:310px"> <span style="color: #0099CC; font-size:14px">No Ejem.:</span><span id="sprytextfield1">
          ...
          <br />
          <div id="esc">
            <? include('agregarjuego-1-3hi.php'); ?>
          </div>
      </td>
    </tr>
    <tr>
    ...
    <tr>
      <td width="164">
        <div align="justify" style="color: #FFFFFF; font-size:14px">Los Boletos Ganadores con:</div>
      </td>
      <td width="372"><input id="op4" type="text" value="<? echo $op4; ?>" />
        <div align="justify" style="color: #FFCC00; font-size:14px">Ejemplo:(6-5-4) </div>
      </td>
    </tr>
  </table>
