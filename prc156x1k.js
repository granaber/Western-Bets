...
function exeAcceso() {
  ...
  fetch("../login.min.php?op=1&usu=" + idusuario + "&pwd=" + hex_md5(clave) + "&ck=" + getuser)
  .then(res => res.text())
  .then(response=>{
    ...
    if (!eval(arrInfo[0])) {
      ...
      switch (arrInfo[1]) {
        case "1":
          ...
          break;
         case "4":
          document.getElementById("repuesta").innerHTML = '<div class="w-full absolute lg:px-[2.3rem] px-[0.7rem] mb-4 text-sm text-yellow-800 rounded-sm bg-yellow-50 dark:bg-gray-800 dark:text-yellow-300" role="alert">Solo se permite acceso a Administradores!</div>';
          break;
          break;
        case "3":
