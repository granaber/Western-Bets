...
if ($op == 2) :
	...
	$result = mysqli_query($GLOBALS['link'], "SELECT * FROM _tabladetablascnf where  IDCN=" . $IDCN . " and Carr=" . $carr);

	if (mysqli_num_rows($result) == 0) :
		$result = mysqli_query($GLOBALS['link'], "Insert  _tabladetablascnf values (" . $IDCN . "," . $carr . "," . $estatus . ")");
	else :
		$result = mysqli_query($GLOBALS['link'], "Update  _tabladetablascnf  set Estatus=" . $estatus . " where  IDCN=" . $IDCN . " and Carr=" . $carr);
	endif;
