<head>
  ...
  $result3 = mysqli_query($GLOBALS['link'], "SELECT * FROM _tconfjornadahi where Estatus=1 and fecha='" . Fechareal($minutosa, "d/n/Y") . "' order by IDCN");
  ...
  if (mysqli_num_rows($result3) != 0) :
    ...
    $result3 = mysqli_query($GLOBALS['link'], "SELECT * FROM _cierrehi where IDJug=" . $tj . " and IDCN=" . $row3["IDCN"]);
  ...
  <div align="left">
    ...
    <div id="box_j" style="background:<?php echo $row["Color"] ?>">
      <table width="590" border="0" cellpadding="0" cellspacing="1">
        <tr bgcolor="">
          <th colspan="19" align="left" bgcolor="<?php echo $row["Color"] ?>" scope="col"><span id='tj' title="<?php echo $tj; ?>" style="color:#FFFFFF; font-size:16px"> <br /><?php echo $row["Descrip"] ?></span> <?php if ($masde == true) :
        ...
        <tr bgcolor="<?php echo $row["Color"] ?>">
          <th colspan="19" align="left" valign="middle" scope="col"><?php if ($idc == '-2' || $idc == '-1') : ?> Concesi.:
              <input id="cons" type="text" size="10" maxlength="10" onkeyup="this.value = this.value.toUpperCase();validar_con_ju(event,1,'nom');" onfocus="$('nom').disabled='disabled';$('valida').disabled='disabled';$('ejem').disabled='disabled';" onchange="" />
        ...
        <tr>
          ...
          for ($i = 1; $i <= $row["CantidadCarr"]; $i++) {
            ...
            echo '<th width="17" bgcolor=' . $row["Color"] . ' title="' . $cejem . '" style="border:medium" >' . $i . '.-</th>';
            echo '<th  style="border:medium"  bgcolor="#FFFFFF" ><img src="media/marcar_todos_blanco.bmp" onclick="celdasicons(' . $i . ',' . $row["calculo"] . ');" /></th>';
            //print_r($retirado);
            //echo $cejem;
            //echo $_xc[$i-1]-1;
            for ($j = 1; $j <= $row["EjemxCarr"]; $j++) {
              ...
              echo '<th id="celda' . $i . '' . $j . '" width="13" bgcolor="' . $activo . '" title="' . $numk . '"  style="border:medium"  onclick="cambiarcelda(event,' . $row["calculo"] . ',' . $i . ');"><span class="esbk">' . $j . '</span></th>';
            }
