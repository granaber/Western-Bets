	...
	if ($activo1 == 1) :
		if ($row["Tandas"] == 1) :
			for ($r = 0; $r <= count($_xc) - 2; $r++) {
				$result4 = mysqli_query($GLOBALS['link'], "SELECT * FROM _cierre where  IDJug=" . $tj . " and ct=" . $_xc[$r] . " and IDCN=" . $idcn);
			...
			for ($r = 0; $r <= count($_xc) - 2; $r++) {
				...
				if ($ct == $row["CantidadCarr"]) :
					$result4 = mysqli_query($GLOBALS['link'], "SELECT * FROM _cierre where  IDJug=" . $tj . " and ct=" . $tt . " and IDCN=" . $idcn);
					if (mysqli_num_rows($result4) == 0) :
