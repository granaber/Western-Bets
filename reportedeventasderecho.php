...

{
...

	{
		...
		$this->text(10, 7, 'Reporte Resumido de Ventas JUGADA POR DERECHO');
		...
		if ($clsi == 2) :
			if ($grupo != $row['IDG']) :
				if ($grupo != 0) :
					...
					$aa[6] = number_format(($totalventasGD - $totalporcentajeGD) - $totalpremioGD, 2, '.', '');
		...
		if (mysqli_num_rows($result2) != 0) :
			...
			switch ($clsi) {
				case 1:
					...
					if ($pdv1 == -3) :
						$aa[4] = number_format($totalgporce, 2, '.', '');
					else :
						$aa[4] = number_format($totalgporce1, 2, '.', '');
					endif;
