...
var AudTik;
const [RETIRO,RECARGA] = [1,2] // ret,pay

function captarOdds(idj) {
  var larre = new Array(),
    selcas = new Array();

  for (var i = 0; i <= lista_id.length - 1; i++) {
    if ($("ckec" + lista_id[i]).checked) {
      var nvl = lista_id[i].split("-");
      larre.push(nvl[1] + "-" + idj);
    }
  }
  if (larre.length != 0) {
    for (var i = 0; i <= listacs_id.length - 1; i++) {
      if ($("casino-" + listacs_id[i]).checked) selcas.push(listacs_id[i]);
    }
    if (selcas.length === 0 || selcas.length === listacs_id.length) vselcas = 0;
    else vselcas = selcas.join(",");
    makeResultwin("skynet-3xtNw.php?liga=" + larre.join(",") + "&casino=" + vselcas + "&idb=1", "coc");
    // console.log('skynet-3xtNw.php?liga='+larre.join(',')+'&casino='+vselcas+'&idb=1')
  } else {
    alert("Disculpe, pero no seleccion ningun deporte a captar!");
  }
}
function onclick_sport(id, parcial) {
  if (id != "0" && !parcial) $("ckec" + id).checked = !$("ckec" + id).checked;
  else {
    for (var i = 0; i <= lista_id.length - 1; i++) {
      if (parcial) {
        var nvl = lista_id[i].split("-");
        if (parseInt(nvl[0]) == parseInt(id)) $("ckec" + lista_id[i]).checked = !$("ckec" + lista_id[i]).checked;
      } else $("ckec" + lista_id[i]).checked = !$("ckec" + lista_id[i]).checked;
    }
  }
}
function onclick_casino(id) {
  if (id != 0) $("casino-" + id).checked = !$("casino-" + id).checked;
  else {
    for (var i = 0; i <= listacs_id.length - 1; i++) {
      $("casino-" + listacs_id[i]).checked = !$("casino-" + listacs_id[i]).checked;
    }
  }
}
...
function filterBy() {
  ...
  for (y = 0; y < mygrid.getRowsNum(); y++) {
    ...
    else mygrid.setRowHidden(mygrid.getRowId(y), true);
  }
}
function filterBy1() {
  ...
  for (y = 0; y < mygrid1.getRowsNum(); y++) {
    ...
    else mygrid1.setRowHidden(mygrid1.getRowId(y), true);
  }
}
function filterBy2() {
  ...
  for (y = 0; y < mygrid2.getRowsNum(); y++) {
    ...
    else mygrid2.setRowHidden(mygrid2.getRowId(y), true);
  }
}
...
function makeResEsp2(url, obj, id) {
  ...
  new Ajax.Request("jornada-1.php?opc=" + id, {
    method: "get",
    onComplete: function (transport) {
      var response = transport.responseText;
...
function calcular(g) {
  ...
  document.form1.Total.value =
    ...
    document.form1.v2.value *
    document.form1.v3.value *
    document.form1.v4.value *
    document.form1.v5.value *
    document.form1.v6.value;
}
...
function grabar_consecionario() {
  if ($("c_idc").value != "") element1 = $("c_idc").value + $("grupo").value;
  else element1 = "";
      ...
      $("moneda").value,
    {
      ...
      onSuccess: function (transport) {
        ...
        if (response[0]) {
          ...
          switch (response[1]) {
            case 0:
              nalert("ERROR.", "NO SE PUEDE ACTUALIZAR /nINTENTE DE NUEVO!");
              break;
            case 1:
              nalert("ERROR.", "LA LETRA NO PUEDE ESTAR EN BLANCO!");
              break;
            case 2:
              nalert("ERROR.", "EL NOMBRE NO PUEDE ESTAR EN BLANCO!");
              break;
            case 3:
              nalert("ERROR.", "LA DIRECCION NO PUEDE ESTAR EN BLANCO!");
              break;
            case 4:
              nalert("ERROR.", "EL ESTADO NO PUEDE ESTAR EN BLANCO!");
              break;
            case 5:
              nalert("ERROR.", "EL MUNICIPIO NO PUEDE ESTAR EN BLANCO!");
              break;
            case 6:
              nalert("ERROR.", "LA LETRA YA EXISTE");
              break;
          }
        }
      },
...
function grabar_usuario() {
  ...
  if ($("vbanca").checked) valor2 = $("IDB").value;
  else valor2 = 0;
      ...
      $("usu").title,
    {
      ...
      onComplete: function (transport) {
        ...
        if (response[0]) {
          ...
          switch (response[1]) {
            case 1:
              nalert("ERROR", "NO GENERO LA CLAVE PARA ESTE NUEVO USUARIO");
              break;
            case 2:
              nalert("ERROR", "INTENTE DE NUEVO NO PUEDO GRABAR EL USUARIO");
              break;
            case 3:
              nalert("ERROR", "ESTE USUARIO YA EXISTE!");
              break;
          }
        }
      },
...
function elimnar_usuario() {
  ...
  if (desci == true) {
    ...
    new Ajax.Request("usuario-1-2.php?idu=" + element1 + "&pr=2&idt=" + $("usu").title, {
      method: "get",
      asynchronous: false,
      onComplete: function (transport) {
        var response = transport.responseText.evalJSON(true);
        dhxWins1.window("w1").close();
        dhxWins1.window("w2").close();
        if (response[0]) nalert("ACTUALIZACION", "USUARIO ELIMINADO");
        else nalert("ERROR", "NO PUEDO ELIMINAR EL USUARIO");
    ...
    new Ajax.Request("grupo-1-2.php?idg=" + element1 + "&pr=2", {
      ...
      onComplete: function (transport) {
        ...
        if (response) {
          ...
          makeRequestSP("grupo-1-1.php");
        } else nalert("ERROR", "DISCULPE NO PUEDO ELIMINAR ESTE GRUPO!!");
      },
...
function grabar_hipo() {
  ...
  new Ajax.Request(
    "hipodromo-1-2.php?nm=" + element1 + "&es=" + element2 + "&idh=" + element3 + "&sg=" + element4 + "&pr=1",
    {
      method: "get",
      onComplete: function (transport) {
        var response = transport.responseText;
...
function jsonvalores(oEvent) {
  ...
  new Ajax.Request("validaformgamejornadabb.php?" + oEvent + "=" + txtfield, {
    ...
    onSuccess: function (transport) {
      ...
      $("fromResultadosDeportes").innerHTML = response.dispatch;
    },
...
function cargarpartidos(Idj) {
  if (Idj == -1) desicion = parseInt($("cant_p").value) != 0;
  else desicion = true;
  ...
  if (desicion) {
    ...
    new Ajax.Request(
      "jornadabb.php?cant=" + element1 + "&idj=" + element2 + "&dp=" + element3 + "&IDB=" + IDB + "&fc=" + fc,
      {
        method: "get",
        onComplete: function (transport) {
          var response = transport.responseText;
  ...
  } else nalert("ERROR", "INDIQUE LA CANTIDAD DE PARTIDOS");
}
...
function blkOdds(campos, part, IDDD, grupo) {
  var icampos = campos.split(",");
  var me = "habiChk-" + IDDD + "-" + part;
  for (i = 0; i <= icampos.length - 1; i++) {
    if ($(me).checked) $(icampos[i]).setAttribute("disabled", "true");
    else $(icampos[i]).removeAttribute("disabled");
  }
  if ($(me).checked) $("obc" + IDDD + "" + part).setAttribute("disabled", "true");
  else $("obc" + IDDD + "" + part).removeAttribute("disabled");
}
function guardarjorbb(liga, auto) {
  if (VerificarLogros(false)) {
    ...
    codigosDB = "";
    for (i = 1; i <= cant; i++) codigosDB += $("CodigoEq1_" + i).value + "|" + $("CodigoEq2_" + i).value + "|";
    ...
    for (i = 0; i <= element5 - 1; i++) {
      ...
      for (j = 1; j <= arrdev.length - 1; j++) {
        if (sudgrupo == $(arrdev[j]).lang) {
          ...
          blk = $("habiChk-" + sudgrupo + "-" + (i + 1)).checked ? 1 : 0;
          elemd[i] += sudgrupo + "-" + blk + "[" + subg + "*";
    ...
    new Ajax.Request("jornadabb-1-2.php", {
      parameters: {
        op: 1,
        idc: element1,
        lista: listado,
      ...
      onSuccess: function (transport) {
        ...
        if (!response) nalert("ERROR", "NO SE ACTUALIZO LA INFORMACION INTENTE DE NUEVO");
      },
      onCreate: function () {
        mientrasProceso("Jornada Grabada", "Grabando Jornada");
      },
      onFailure: function () {
        alert("No tengo respuesta Comuniquese con el Administrador!");
      },
...
function orden(n) {
  ...
  http_request3.open(
    "GET",
    "ordenusu.php?&opc=" + n + "&bq=" + element1 + "&ord=" + element2 + "&bq2=" + element3,
    true
  );
...
function imprimirre() {
  ...
  if (idj != "") {
    ...
    if ($("rt2").checked == true) {
      abrir(
        "reportedeventasresumido.php?gp=" +
          gp +
          "&d1=" +
          d1 +
          "&d2=" +
    ...
    onComplete: function (transport) {
      ...
      if (!eval(arrInfo[0])) {
        ...
        if (tp != 4 && tp != 5) {
          ...
          if (tp == 2) {
            $("carre_v").disabled = "disabled";
          }
          if (tp == 1) {
            $("nom").disabled = "disabled";
          }
        } else {
          for (iii = 1; iii <= parseInt($("carr").title); iii++) {
            $("v" + iii).disabled = "disabled";
          }
          if (tp == 4) {
            $("nom").disabled = "disabled";
          }
...
var impresion = "";
function formatclick(se, gp, imprimir, cambio, nuevo, mnd) {
  ...
  a =
    '<table  border="0" style="font-family:Arial, Helvetica, sans-serif; font-size:9px;margin-top: 5px;margin-left: 2px;width: 100%;"> <tr style="background: #ccc;">    <th  scope="col" width="10"  align="left"  style="font-size: 10px;color: #000;" >Equipos</th>    <th width="10" scope="col"  style="font-size: 10px;color: #000;">Logro</th>  </tr>';

  //Serialbb
  b =
    '<table  border="0" style="font-size:12px" align="left;color:#666666"> <tr>  <th colspan="3" scope="col"  ><p align="center">..::_Tu Jugada Online_::.</p></th> </tr><tr><th colspan="3" scope="col" align="center"> k******^ TICKET VALIDO *^*^***k</th></tr> <tr>   <th colspan="3" scope="col" >Serial ticket:' +
    serialdedd +
    '</th> </tr>   <th colspan="3" scope="col" ><p align="left">Agencia:' +
    conces +
    '</p></th> </tr><tr>   <th colspan="3" scope="col" >Fecha:' +
    ff +
    " Hora:" +
    hh +
    '</th> </tr> <tr><tr>    <th  scope="col">N Partido</th> <th  scope="col" >Partido</th>    <th scope="col">Total</th>  </tr><tr><th colspan="3"   style=" font-size:10px" scope="col">#-----------------------------------#</th></tr>';
  ...
  if (true) {
    ...
    for (j = 1; j <= parseInt($("totaldefilar").lang) - 1; j++) {
      ...
      if ($("part" + i + "_" + gp_x).lang != 0 && $("rro" + i + "_" + gp_x).style.display != "none") {
        ...
        if (teaimp.length == 1) toppp = 0;
        else toppp = teaimp.length - 2;
        ...
        for (ii = 0; ii <= toppp; ii++) {
          ...
          if (isNaN(valocc) == true) {
            ...
            if (valocc1 < 0) {
              ...
              ap = factor * ap;
              valorini = ap;
              var isSign = valocc.split("").find((o) => o === "+");
              if (isSign) de = valocc;
              else de = "+" + valocc;
            }
            //*********************************************************************************
          ...
          dequi = equi.split("||");
          idddr = teaimp2[ii].split("%"); // ***************************************

          if (mpe == 0) {
            mpe = parseInt(dequi[0]);
            idddc = parseInt(idddr[0]);
          }

          lequi += parseInt(dequi[0]) + "|";
          liddd += parseInt(idddr[0]) + "|";
          ...
          a +=
            '<tr><th align="left"  style=" font-size:10px">' +
            dequi[1] +
            '</th><th align="right"  style=" font-size:10px"> ' +
            de +
            " </th></tr>";
          //$(dequi[0]).innerHTML+'-'+
    ...
    a +=
      '<tr><th colspan="2"   style="    background: #bab8b8;"  scope="col"></th></tr><tr><th  style=" font-size:10px;font-weight: bold" align="left" style="font">Apuesta</th><th  style=" font-size:10px"  align="right">' +
      mnd +
      $("ap").value +
      '</th> </tr><tr><th align="left"  style=" font-size:10px" >A Cobrar:</th> <th  style=" font-size:10px" align="right">' +
      mnd +
      valorini +
      '</th> </tr>  <tr><th align="left"  style=" font-size:10px">Ganacia:</th><th align="right"  style=" font-size:10px">' +
      mnd +
      redond(valorini - parseFloat($("ap").value), 2) +
      "</th>  </tr>";

    a +=
      '<tr><th colspan="3" scope="col" align="center" style="font-size:12px" > Toda apuesta realizada despues de iniciar un partido sera nula.</th></tr><tr></tr>';
    ...
    b1 = "";
    b1 +=
      '<tr>  <th colspan="3" scope="col"  ><p align="left">-------------------------------------</p></th> </tr><tr><th colspan="3" scope="col" align="left">Precio  :' +
      mnd +
      $("ap").value +
      ' </th> </tr><tr><th colspan="3" scope="col" align="left">Premio  :' +
      mnd +
      valorini +
      '</th> </tr>  <tr>  <th colspan="3" scope="col" align="left"> Su Ganacia  :  ' +
      mnd +
      redond(valorini - parseFloat($("ap").value), 2) +
      ' </th>  </tr><tr>  <th colspan="3" scope="col" align="left">-------------------------------------</th>  </tr>';
    ...
    $("printer").innerHTML =
      b +
      '<th>Segurida:</th> <th colspan="2">' +
      se +
      '</th> </tr><tr><th colspan="3" scope="col" align="center"> ****** VERIFIQUE SU JUGADA *****</th></tr></table>';
...
function impticketbb(modo, mnd) {
  if (Number($("ap").value) != 0) {
    terminal = $("est").title;
    IDusu = $("usu").title;
    if (isset("IDC")) idc = $("IDC").value;
    else idc = $("con").title;
    if (idc != "") {
      idj = $("idj_x").lang;
      estatus = true;
        ...
        do {
          ...
          if (desci == true) {
            if (ent == true) {
              ...
              tdj = tdj.replace(/%/g, ")");
              var erro = false;
              new Ajax.Request("grabarjugadabbnekx2.php", {
                parameters: {
                  op: 1,
                  serial: serialdedd,
                  jgd: tdj,
                ...
                onSuccess: function (transport) {
                  ...
                  if (response[0]) {
                    ...
                    switch (modo) {
                      case 0:
                        formatclick(response[1], 0, true, response[3], vretur, mnd);
                        break;
                      case 1:
                        formatclick_msg(response[1], 0, true, response[3], vretur, mnd);
                        break;
                    }
                    _fnactCredito(idc);
                  } else {
                    switch (response[1]) {
                      ...
                      case "4":
                        nalert(
                          "TOPE MAXIMO",
                          "4:EL MONTO DE LA APUESTA QUE HA COLOCADO NO ESTA PERMITIDA \nCON LA CANTIDAD DE PARTIDOS SELECCIONADOS!"
                        );
                        break;
                      case "5":
    ...
    } else nalert("ERROR", "Debe Seleccionar la Letra a Realizar El Ticket!!");
  } else nalert("ERROR", "No hay monto en la Apuesta!");
}
...
function valiDD(e, t, o, gp, IDB, oEvent, tablaID, mnd) {
  /*  oEvent= oEvent || window.event;
	     var txtfield = oEvent.target || oEvent.srcElement;	*/
  ...
  if (vrc[0] >= 2) {
    ...
    $("tl" + gp).title = o;
    colunn = sercol.split("|");
    if (t == 0 && colunn.length == 2) xt = false;
    else xt = $(colunn[1 - t] + "_" + e).checked == true;
    ...
    if (permitir(vrc[vrc.length - 1], e, gp, o) == true) {
      ...
      formatclick("0", gp, false, 0, 0, mnd);
    } else {
      $(colunn[t] + "_" + e).checked = false;
...
function updatelogro(iddd, fila, cmp, gp, IDB, txtfield, tablaID) {
  ...
  if ($("forttD_" + idg).style.display != "none")
    //,asynchronous:false
    new Ajax.Request("procierre.php?op=11&idj=" + idj + "&idg=" + idg + "&IDB=" + IDB + "&IDusu=" + $("usu").title, {
      method: "post",
      onComplete: function (transport) {
        var response = transport.responseText.evalJSON(true);
        ...
        if (isset(tcoll[ii] + "_" + fila)) {
          if ($(tcoll[ii] + "_" + fila).checked == true) {
            ...
            $("part" + fila + "_" + gp).title +=
              ...
              "$";
            $("part_2_" + fila + "_" + gp).title +=
              vv[1] + "%" + $("a2_" + vv[1] + "_" + vv[0] + "_" + fila).title + "$";
...
function ecruteveri(fila, cp, gp) {
  ...
  for (iii = 0; iii <= ncb.length - 2; iii++) {
    if (isset("nc_" + ncb[iii] + "_" + gp)) {
      ...
      for (ii = 0; ii <= tcoll.length - 1; ii++) {
        if (tcoll[ii].search("-") == -1) {
          if (isset(tcoll[ii] + "_" + fila)) if ($(tcoll[ii] + "_" + fila).checked == true) mp = false;
        }
      }
    }
  }
  ...
  if (eventodb == 1 && tecla_especial == false) {
    ...
    if (txtfield.id == camposv1) {
      if (permitidos.indexOf(caracter) != -1) {
        $(camposv2).value = $(camposv1).value + caracter;
        $(camposv2).value = -1 * Number($(camposv2).value);
      }
    }
    if (txtfield.id == camposv2) {
      if (permitidos.indexOf(caracter) != -1) {
        $(camposv1).value = $(camposv2).value + caracter;
        $(camposv1).value = -1 * Number($(camposv1).value);
      }
    }
  }
...
function copiarvaloresjsonbb(oEvent) {
  ...
  for (ii = 1; ii <= cant; ii++) $(cmpo + ii).value = valorop;
}
...
function cierreautoajax(op, idj) {
  new Ajax.Request("procierre.php?op=7&idj=" + idj + "&idu=" + $("usu").title, {
    ...
    onFailure: function () {
      $("horaser").innerHTML = "NO HAY COMUNICACION";
    },
  });
}
...
function Activar_Ventas(idp, idj, IDB) {
  new Ajax.Request("ticketbb-3.php?xidj=" + idj + "&idp=" + idp + "&tdl=" + $("forttD_" + idp).lang + "&IDB=" + IDB, {
    method: "post",
    onComplete: function (transport) {
      var response = transport.responseText;
...
function activarcc(idp, idj, IDB) {
  ...
  if ($("forttD_" + idp).style.display == "none")
    new Ajax.Request("procierre.php?op=11&idj=" + idj + "&idg=" + idp + "&IDB=" + IDB, {
      method: "post",
      asynchronous: false,
      onComplete: function (transport) {
        var response = transport.responseText.evalJSON(true);
  ...
  if (op == 1) {
    evve = new Ajax.PeriodicalUpdater("", "procierre.php?op=2&idj=" + idj + "&dp=" + dp, {
      ...
      onSuccess: function (transport) {
        ...
        if (ultimo != 0) Ext.example.msg("Cerrado", "Partido cerrado (" + ultimo + ")");
      },
      onFailure: function () {
...
// function calertMsg(msg){
// 	$('alertmsg-time').innerHTML=msg;
// 	$('alertmsg').style.display="";
// 	setTimeout(3000,closeMsg())
// }
// function closeMsg(){
// 	$('alertmsg').style.display="none";
// 	$('alertmsg-time').innerHTML='';
// }
...
function LogroVista(fileout) {
  ...
  abrir(
    fileout + "?idj=" + v + "&idg=" + lista + "&usu=" + $("con").title + "&IDB=" + $("IDB").value,
    "Reporte de Logros ",
    1,
    0,
    0,
...
function camposImpLogMOD(cal) {
  ...
  /////////////////////////////////////////////////////////////////
  new Ajax.Request("logrosimprimirNK.php", {
    parameters: { xfc: field.value },
    method: "get",
    onComplete: function (transport) {
      var response = transport.responseText;
...
function formatoticket(se) {
  ...
  new Ajax.Request("procierre.php?op=4&serial=" + se, {
    ...
    onComplete: function (transport) {
      ...
      a ='<section id="content-ticket-'+se+'" style="display: flex;flex-wrap: nowrap;justify-content: center;">'+
      (Number(valoreex[6])!==1?' <h1 class="text-anul-ticket">ANULADO</h1>':'')+'<table width="235" border="0" style="font-size:11px; font-family:sans-serif, Arial;line-height: 18px;margin-top:5px"> <tr>   <th colspan="3" scope="col" >' +
        tti +
        ' Ticket No:' +
        '<span style="font-size:13px;font-weight: bold;" >'+
        se +'</span>' +
       '</th> </tr><tr style="background: #ccc;font-weight: bold;">    <th width="5" scope="col" style="font-weight:bold">Jgo.</th> <th  scope="col" width="78" style="font-weight:bold">Equipos</th>    <th width="55" scope="col" style="font-weight:bold">Logro</th>  </tr>'


      b =
        '<table  border="0" style="font-size:18px" align="left" style="color:#666"> <tr>  <th colspan="3" scope="col"  ><p align="center" style="color:#666"> ..::Tu Jugada Online::. </p></th> </tr><tr><th colspan="3" scope="col" align="center"> ^**^***^ TICKET NO VALIDO *^*^***^</th></tr> <tr>   <th colspan="3" scope="col" style="color:#666">No. Ticket:' +
        se +
        '*****COPIA*****</th> </tr> <tr>   <th colspan="3" scope="col" >Fecha:' +
        valoreex[1] +
        " Hora:" +
        valoreex[0] +
        '</th> </tr> <tr>   <th colspan="3" scope="col" ><p align="left">Agencia:' +
        conces +
        '</p></th> </tr><tr>   <th colspan="3" scope="col" >' +
        tti +
        '</th> </tr><tr>    <th  scope="col">Hora</th> <th  scope="col" >Partido</th>    <th scope="col">Total</th>  </tr><tr><th colspan="3" scope="col"><p align="left">--------------------------------------</p></th></tr>';
      ...
      for (i = 1; i <= arl.length - 2; i++) {
        ...
        switch (r_g[i - 1]) {
          case 1:
            Tescrute = '<span style="color: #107516;">-GANO</span>'
            break
          case 0:
            Tescrute = '<span style="color: #dc3545;">-PERDIO</span>'
            break
          case 2:
            Tescrute = '-S/E'
            break
        ...
        a +=
          '<tr><th scope="col" > ' +
          dequi[1] +
          '</th><th scope="col" width="78" class="alinl"  >' +
          dequi[2] +
        ...
        b +=
          '<tr><th scope="col">' +
          dequi[1] +
          '</th><th scope="col" align="left" >   ' +
          dequi[2] +
          '</th><th scope="col"  align="right">  ' +
          dequi[3] +
          Tescrute +
          "</th></tr>";
      }
      ...
      a +=
      '<tr style="background: #ccc;height: 10px;"><th colspan="3" scope="col"><p align="left"></p></th></tr><tr><th  scope="col" style="font-weight: bold;">APUESTA:</th><th scope="col"  colspan="1"><p align="right">' +
        valoreex[5] +
        valoreex[2] +
        '</p></th> </tr><tr><th  scope="col" style="font-weight: bold;">A Cobrar:  </th> <th scope="col" colspan="1" ><p align="right">' +
        valoreex[5] +
        valoreex[3] +
        '</p></th> </tr>  <tr><th  scope="col" style="font-weight: bold;" >Su Ganacia:</th><th scope="col" colspan="1"><p align="right">' +
        valoreex[5] +
        redond(parseFloat(valoreex[3]) - parseFloat(valoreex[2]),2) +
        "</p></th>  </tr>";

      b +=
        '<tr>  <th colspan="3" scope="col" align="left" >-------------------------------------</th> </tr><tr><th colspan="2" scope="col" align="left">Precio  : </th><th scope="col" align="right">' +
        valoreex[5] +
        valoreex[2] +
        '</th> </tr><tr><th colspan="2" scope="col" align="left">Premio  :</th> <th scope="col" align="right">' +
        valoreex[5] +
        valoreex[3] +
        '</th> </tr>  <tr>  <th colspan="2" scope="col" align="left">Su Ganacia  :</th> <th scope="col" align="right">' +
        valoreex[5] +
        redond(parseFloat(valoreex[3]) - parseFloat(valoreex[2]),2) +
        "</th>  </tr>";
      ...
      b +=
        '<tr><th colspan="3" scope="col" align="center"> DE LOS ' + $("cngfvendedor").lang + " MIN</th></tr><tr></tr>";

      $("printer2").innerHTML = a + "</table></section>";
      $("section2").innerHTML=`<div id="list-btn-${se}" class="btn-command-ticket">`+(Number(valoreex[6])===1?`<button onclick="eliminarticketbb(${se},this,0)" type="button"  class="btn btn-danger btn-sm">Anular</button><button type="button" class="btn btn-success btn-sm">Reenviar</button>`:`<div></div>`)+`</div>`;

      //$("printer").style.display="none";

      // $("printer").innerHTML =
      //   b +
      //   '<th>SEGURIDAD:</th> <th colspan="2">' +
      //   valoreex[4] +
      //   '</th> </tr><tr><th colspan="3" scope="col" align="center">****** VERIFIQUE SU JUGADA *****</th></tr><tr>- </tr><tr>- </tr><tr>- </tr><tr>- </tr><tr>- </tr><tr>- </tr></table>';
    },
...
function ByViewDUKForAnimalitos(se,st,usu)
	{

      new Ajax.Request('animalitos/vertk.php?serial='+se+"&TU=1",{ method:'get',asynchronous:false,	onComplete: function(transport){
      //new Ajax.Request('animalitos/grabar_animalitos.php',{ parameters: {Jugada:Object.toJSON(TicketCloud),IDC:$('con').title,usu:$('usu').title},method:'post',asynchronous:false,	onComplete: function(transport){
                  const regexp = /anulado/g;
                  const imgtk = `img-state-${se}`
                  const urlname = $(imgtk).src
                  const array = [...urlname.matchAll(regexp)];

                  const state = array.length ? 4 : 0

                  var response = transport.responseText ;
                   $('printer2').innerHTML='<section id="content-ticket-'+se+'" style="display: flex;flex-wrap: nowrap;justify-content: center;">'+(Number(state)===4?' <h1 class="text-anul-ticket">ANULADO</h1>':'')+response+'</section>';
                   $("section2").innerHTML=`<div id="list-btn-${se}" class="btn-command-ticket">`+(Number(state)!==4?`<button onclick="eliminarticketAN(${se},${usu})" type="button"  class="btn btn-danger btn-sm">Anular</button><button type="button" class="btn btn-success btn-sm">Reenviar</button>`:`<div></div>`)+`</div>`;

                  },
                onFailure: function(){ alert('No tengo respuesta Comuniquese con el Administrador!'); }
                 });
	}
  function ByViewDUKForAmericanas(se,st,usu)
	{

      new Ajax.Request('americanas/verticket.php?serial='+se+"&TU=1",{ method:'get',asynchronous:false,	onComplete: function(transport){
      //new Ajax.Request('animalitos/grabar_animalitos.php',{ parameters: {Jugada:Object.toJSON(TicketCloud),IDC:$('con').title,usu:$('usu').title},method:'post',asynchronous:false,	onComplete: function(transport){
                  const regexp = /anulado/g;
                  const imgtk = `img-state-${se}`
                  const urlname = $(imgtk).src
                  ...
                  const parameter = {
                    ...
                    se: response[3],
                    direcc: response[4],
                    Hipodromo: response[5],
                    carr: response[6],
                    op: response[8]?2:1,
                    premio: response[9],
                    dividendo: response[11],
                    llegada:response[10],
                    iHorse: response[7],
                    frm: true,
                    Valor_J:response[14],
                  ...
                  const data = ticketimpri(parameter)

                   $('printer2').innerHTML='<section id="content-ticket-'+se+'" style="display: flex;flex-wrap: nowrap;justify-content: center;">'+(Number(state)===4?' <h1 class="text-anul-ticket">ANULADO</h1>':'')+data+'</section>';
                   $("section2").innerHTML=`<div id="list-btn-${se}" class="btn-command-ticket">`+(Number(state)!==4?`<button onclick="eliminarticketAM(${se},${usu})" type="button"  class="btn btn-danger btn-sm">Anular</button><button type="button" class="btn btn-success btn-sm">Reenviar</button>`:`<div></div>`)+`</div>`;
  ...
  function ticketimpri({
    ...
    if (frm)
      tabl = [
        '<table  border="0">  <tr>    <td colspan="4" a><p align="center" style ="padding: 15px;font-weight: bold;">', //0
        '</p></td>  </tr>    <tr>    <td >', //1
        '</td>', //2
        '<td colspan="3" style="font-weight: bold;">', //3
        '</td>  </tr>  <tr>    <td>PuntoV.:</td>    <td colspan="3">', //4
        '</td>  </tr>  <tr>    <td colspan="4">Fecha:', //5
        ' &nbsp;Hora:', //6
        '</td>     </tr>    <tr><td colspan="4"  style="padding-top:5px">Valido:', //7
        '</td>  </tr><tr>    <td colspan="4" style="padding-top:5px">Carr:', //8
        '</td>  </tr> ', //9
        ' <tr>   <td >', //10
        '</td>    <td>', //11
        '</td>  </tr>', //12
        '<tr style="background: #d9d6d6;"><td colspan="2" style ="padding: 10px;">Total(' + mnd + '):</td>    <td colspan="2" style ="padding: 10px;"><samp  align="right"  >', //13
        '</samp></td>  </tr>    <tr><td colspan="4"></td>  </tr></table>', //14
        '</samp></td>  </tr><tr>    <td colspan="2">Conforme:</td>    <td colspan="2"><samp  align="right">', //15
        '</samp></td>  </tr>  <tr>    <td colspan="2">Premio(' + mnd + '):</td>    <td colspan="2"><samp  align="right">', //16
        '</samp></td>  </tr>    <tr><td colspan="4">Todas las apuestas realizadas despues </td>  </tr><tr><td colspan="4">de la hora de partida seran anuladas. sin excepcion.</td>  </tr></table>', //17
        '<tr><td colspan="4" >' //18
      ]
    else
      tabl = [
        ...
        '</div>',
        '<div>Total(' + mnd + '):',
        '</div><div>Todas las apuestas realizadas despues</div><div>de la hora de partida seran anuladas. sin excepcion.</div><div>-</div><div>-</div><div>-</div><div>-</div><div>-</div><div>-</div><div>-</div><div>-</div><div>-</div>',
        '</div>',
        '<div>Dividendo:',
        '</div><div>Premio(' + mnd + '):'
      ]
    ...
    var a = ''
    a = tabl[0] + 'Americanas' + tabl[1] + tabl[2]
    ...
    a +=
      ...
      Hipodromo +
      ')' +
      tabl[9]+ tabl[18]+'<p style ="padding: 15px;font-weight: bold;text-align: center;">Jugada</p>' + tabl[12] 
    total = 0
    lista = jugada.split('|')
    ...
    for (i = 1; i <= lista.length - 1; i++) {
      ...
      total += parseFloat(apuesta[0]) + parseFloat(apuesta[1]) + parseFloat(apuesta[2])
    }

    if (op == 1) {
      a += tabl[13] + Valor_J + tabl[frm && barrcode ? 17 : 14]
    } else {
      const d = dividendo.split(",")
      const ll = llegada.split(",")
      const post = ["W","P","S"]
      const conforme = d.map((e,i)=>{
        if (e!==''){
          const p = post[i]
          return  `${p}:${ll[i]}(${e})`
        }
      ...
      a += tabl[13] + Valor_J + tabl[15] + conforme + tabl[16] + premio + tabl[frm && barrcode ? 17 : 14]
    }
    if (frm && barrcode) {
      a += '<svg id="barcode"></svg>'
    }
    a += '<br>'
    if (space) {
      maSpace = '-<br>'
      maSpace = maSpace.repeat(space)
      a += maSpace
    }
    ...
    new Ajax.Request("procierre.php?op=5&serial=" + se + "&opadmon=" + admon , {
      ...
      onSuccess: function (transport) {
        ...
        switch (response.status) {
          case 0:
            const imgtk = `img-state-${se}`
            $(imgtk).src='./media/anulado.png'
            alert('Su Ticket fue ANULADO, con exito!')
            destrucWindow()
            break;
...
function eliminarticketAM(se,usu){
  if (confirm('Esta SEGURO DE ANULAR EL TICKET '+se+'?')){
  new Ajax.Request('americanas/AnulaTicket.php?serial='+se+'&usu='+usu,{ method:'get',asynchronous:false,	onComplete: function(transport){
              var response = transport.responseText.evalJSON(true) ;
...
function eliminarticketAN(se,usu){
  if (confirm('Esta SEGURO DE ANULAR EL TICKET '+se+'?')){
  new Ajax.Request('animalitos/Anultk.php?serial='+se+'&usu='+usu,{ method:'get',asynchronous:false,	onComplete: function(transport){
              var response = transport.responseText.evalJSON(true) ;
    ...
    new Ajax.Request("procierre.php?op=5&serial=" + se + "&opadmon=" + admon, {
      ...
      onSuccess: function (transport) {
        ...
        switch (response.status) {
          ...
          case 1:
            modal_myBtn(response.id);
            // alert(
            //   "Este ticket no puede Ser Eliminado, porque el partido ya esta cerrado"
            // );
            ...
            //     break;
            //   case "a2":
            //     mygrid1.setRowColor(se, "#FF3737");
            //     break;
            //   case "a3":
            //     mygrid2.setRowColor(se, "#FF3737");
            //     break;
            // }
...
function makeRequestbb(url, obj) {
  ...
  new Ajax.Request(url + "?fc=" + fecha + "&idc=" + cns, {
    parameters: { idt: $("usu").title },
    method: "get",
    onComplete: function (transport) {
      var response = transport.responseText;
  ...
  if (okey == true) {
    ...
    new Ajax.Request("cfngdeportes.php?op=" + op + "&ldv=" + valores + "&tb=" + tb + "&idt=" + $("usu").title, {
      ...
      onSuccess: function (transport) {
        ...
        if (response) logs(-4, "Escrutes Actulizados", 1);
      },
...
function catcalc_e(e) {
  $('boxescrute').innerHTML =''
  var t = e.date,
    n = $('fc')
  ;(mes = t.print('%m')),
    parseInt(mes) <= 9 ? (mes2 = mes.substring(1, 2)) : (mes2 = mes),
    (n.value = t.print('%d') + '/' + mes2 + '/' + t.print('%Y'))
   
}
function jsonvalores_esc() {
  e  =$('fc').value
  ant_event=null
  new Ajax.Request('escrute-2.php?fc=' +e, {
    method: 'get',
    onComplete: function (e) {
      var t = e.responseText
      ;($('boxescrute').innerHTML = t), t.evalScripts()
    }, onCreate: function() {
      $('boxescrute').innerHTML = '<img src="media/ajax-loader.gif" />'
    },
    onFailure: function () {
      alert('No tengo respuesta Comuniquese con el Administrador!')
    }
...
var ant_event=null
function get_escrute(event,e) {
  if (ant_event){
    ant_event.classList.remove('escruting-button-option-active')
  }else{
    const obs = [...document.querySelector('.escruting-form1').childNodes]
    obs.forEach(o=>{
      if (o.nodeName === 'BUTTON') o.classList.add('escrutin-desactive-all')
    })
    // document.getElementsByClassName('escruting-form1').classList.add('escrutin-desactive-all')
  }
  ant_event=event
  event.classList.add('escruting-button-option-active')
  idbody='select-escrute-from'
  new Ajax.Request(e, {
    method: 'get',
    onComplete: function(e) {
      var t = e.responseText;
      $(idbody).innerHTML = t
      document.getElementById(idbody).classList.add('escrutin-section-2')
      t.evalScripts()
    },
    onCreate: function() {
      $(idbody).innerHTML = '<img src="media/ajax-loader.gif" />'
    },
    onFailure: function() {
      alert('No tengo respuesta Comuniquese con el Administrador!')
    }
...
function makeRequesttk(url) {
  $("tablemenu").innerHTML = "";
  var element = $("tablemenu-odds");
  $("tablemenu-odds").style.display = "none";
  var element2 = $("menu1");
  var fc = $("fch").title;
  $("printer").innerHTML = "";
  cns = $("con").title;

  new Ajax.Request(url + "?fc=" + fc + "&idc=" + cns, {
    parameters: { idt: $("usu").title },
    method: "get",
    onComplete: function (transport) {
      var response = transport.responseText;
  ...
  new Ajax.Request("ticketbb-2.php?fc=" + fc + "&op=1&idc=" + cns, {
    parameters: { idt: $("usu").title },
    method: "get",
    onComplete: function (transport) {
      var response = transport.responseText;
...
function imprimirredd(accesop) {
  ...
  abrir(
    "reportedeventasresumidodd.php?d1=" + d1 + "&d2=" + d2 + "&clsi=" + clsi1 + "&gp=" + gp,
...
function imprimirreddNK(accesop) {
  ...
  abrir(
    "reportedeventasresumidodNK.php?d1=" + d1 + "&d2=" + d2 + "&gp=" + $("grupo").value,
    "Reporte de Ventas Resumido ",
    1,
    0,
    0,
...
function imprimirANULADO(accesop) {
  ...
  abrir(
    "reporterANULADO.php?d1=" + d1 + "&d2=" + d2 + "&clsi=" + clsi1 + "&gp=" + gp,
    "Reporte de Ventas Anulados",
    1,
    0,
    0,
    0,
...
function impri_gana_perdida(accesop) {
  ...
  abrir(
    "reportedeventasganaciasyperdidas.php?d1=" + d1 + "&d2=" + d2 + "&clsi=" + clsi1 + "&gp=" + gp,
    "Reporte de Ganacias y Perdidas Detallado ",
    1,
    0,
    0,
    0,
...
function impri_gana_perdidarsm(accesop) {
  ...
  abrir(
    "reportedeventasganaciasyperdidasrsm.php?d1=" + d1 + "&d2=" + d2 + "&clsi=" + clsi1 + "&gp=" + gp,
    "Reporte de Ganacias y Perdidas Resumido ",
    1,
    0,
    0,
    0,
...
function imprimirredt(accesop) {
  ...
  abrir(
    "reportedeventasdetalladodd.php?d1=" + d1 + "&d2=" + d2 + "&clsi=" + clsi1 + "&gp=" + gp,
    "Reporte de Ventas ",
    1,
    0,
    0,
...
function imprimirredtpv() {
  ...
  abrir(
    "reportedeventasdetalladoddpv.php?d1=" + d1 + "&d2=" + d2 + "&clsi=" + clsi1 + "&gp=" + gp,
    "Reporte de Ventas Punto de Venta ",
    1,
    0,
    0,
    0,
...
function jsonvalores_v8() {
  ...
  if (parseInt($("bserial").value) != 0) {
    tp = 1;
    valor = parseInt($("bserial").value);
    $("bserial").value = 0;
  }

  if (parseInt($("bmonto").value) != 0) {
    tp = 2;
    valor = parseInt($("bmonto").value);
    $("bmonto").value = 0;
  }

  if (parseInt($("bmonto2").value) != 0) {
    tp = 3;
    valor = parseInt($("bmonto2").value);
    $("bmonto2").value = 0;
  }
...
function impresionverticketdd() {
  ...
  abrir(
    "imprimirticketseldd.php?sql=" + sql + "&tipo=" + tipo,
    "Reporte de Ticket ",
    1,
    0,
    0,
...
function imprimi_reporte(nrp) {
  ...
  abrir(
    nrp + "?d1=" + d1 + "&d2=" + d2 + "&clsi=" + clsi1 + "&gp=" + gp,
    "Reporte de Ventas Punto de Venta ",
    0,
    0,
    0,
...
function verestatustabla(op) {
  if (op == 1) {
    ...
    IDJugada = $("tj").title;
    carr = $("carrera").lang;
    veresta = new Ajax.PeriodicalUpdater(
      "",
      "sumatablatotal.php?op=3&IDCN=" + IDCN + "&idc=" + idc + "&idj=" + IDJugada + "&carr=" + carr,
      {
        method: "get",
        decay: 30,
        onSuccess: function (transport) {
...
function calculodeparlay() {
  ap = parseFloat($("apuesta").value);
  valorini = 0;
  for (f = 1; f <= 7; f++) {
    valocc1 = parseFloat($("prl" + f).value);
    // ********************  Calculo del Parlay ***********************************
    if (valocc1 < 0) {
      valorf = valocc1 * -1; // Factor  <0 100/Logro + 1 * Apuesta
      factor = 100 / valorf + 1;
...
function imprimirresultado() {
  var lista = new Array();
  k = 0;
  valormaxi = parseInt($("totalg").lang);
  for (i = 1; i <= valormaxi - 1; i++) {
    if ($("chk0" + i).checked == true) {
      lista[k] = $("chk0" + i).lang;
      k++;
  ...
  v = $("fc").lang;
  if (k != 0) {
    if ($("salida").value == 1) $fileout = "impresionresultados.php";
    else $fileout = "impresionresultadossticket.php";

    abrir(
      $fileout + "?idj=" + v + "&idg=" + lista + "&usu=" + $("con").title,
...
function accesoalsistema() {
  ...
  clave = $("idclave").value;
  $("idusuario").disable = "disabled";
  $("idclave").disable = "disabled";
  $("frm-btn").disable = "disabled";

  new Ajax.Request("login.min.php?op=1&usu=" + idusuario + "&pwd=" + hex_md5(clave) + "&ck=" + getuser, {
    method: "get",
    onCreate: function () {
      $("wait").style.display = "";
    },
    onSuccess: function (transport) {
      ...
      if (!eval(arrInfo[0])) {
        ...
        switch (arrInfo[1]) {
          case "1":
            $("repuesta").innerHTML = '<div class="alert alert-danger" role="alert">Usuario Bloqueado!</div>';
            break;
          case "3":
          ...
          default:
            $("repuesta").innerHTML =
              '<div class="alert alert-danger" role="alert">Su Usuario o Clave es errada!</div>';
        }
        $("idusuario").focus();
      } else {
        $("wait").style.display = "none";
        ...
        window.open("intro.min.php?code=" + Info[1], "_blank");
      }
    },
    onFailure: function () {
      $("idusuario").disable = "";
...
function captural(fc, idj) {
  new Ajax.Request("cpyjndbb.php?fc=" + fc + "&gp=" + $("Grupo").value + "&idj=" + idj, {
    ...
    onSuccess: function (transport) {
      ...
      if (response[0]) {
        if (response[1] == 1) {
          $("resultado").update("No Hay Logros Publicados");
        } else {
    ...
    onCreate: function () {
      mientrasProceso("Logros Copiados", "Copiando...");
    },
    onFailure: function () {
      nalert("ERROR", "No tengo respuesta Comuniquese con el Administrador!");
    },
...
function cierrePartidos() {
  ...
  for (j = 1; j <= parseInt($("totaldefilar").lang) - 1; j++) {
    ...
    if ($("part" + i + "_" + gp_x).lang != 0 && $("rro" + i + "_" + gp_x).style.display != "none")
      verifparti($("ipd" + i + "_" + gp_x).lang, true, false);
  }
}
function verifparti(idp, sincronizacion, mensaje) {
  ...
  new Ajax.Request("procierre.php?op=8&idp=" + idp, {
    method: "get",
    asynchronous: sincronizacion,
    onSuccess: function (transport) {
      var response = transport.responseText.evalJSON(true);
      if (response) {
        if (mensaje) alert("Se Cerraro unos partidos Verifique Por Favor!");
    ...
    if (se != "") {
      ...
      new Ajax.Request("procierre.php?op=9&serial=" + serial + "&se=" + se, {
        ...
        onComplete: function (transport) {
          var response = transport.responseText.evalJSON(true);
          if (response) {
            $("se" + serial).innerHTML = se;
            $("mt" + serial).innerHTML = $("mt" + serial).lang;
            alert("CORRECTO: Ticket Verificado, Puede Pagarlo");
          } else {
            alert("Lo Siento pero el Serial ELECTRONICO No es VALIDO!");
            verif = false;
          }
        },
...
function grabarganadores(op) {
  ...
  new Ajax.Request(
    "ganadores-1-2.php?jn=" + element1 + "&gn=" + ganar + "&cr=" + cierre + "&carr=" + carr + "&op=" + op,
    {
      method: "get",
      asynchronous: false,
      onSuccess: function (transport) {
...
function createByxml(_file, _sql, _campos) {
  new Ajax.Request("newxml.php", {
    parameters: { file: _file, sql: _sql, campos: _campos },
    method: "get",
    asynchronous: false,
    onComplete: function (transport) {
      var response = transport.responseText;
...
function createByxmlForverJugada(_file1, _file2, _file3, _idj) {
  new Ajax.Request("xmlverjugada.php", {
    parameters: { file1: _file1, file2: _file2, file3: _file3, idj: _idj },
    method: "get",
    asynchronous: false,
    onComplete: function (transport) {
      var response = transport.responseText;
...
function grabarBycupoBygrupo(_IDG, _maximoCupo) {
  var respuesta = false;
  new Ajax.Request("grupo-1-2.php", {
    parameters: { IDG: _IDG, maximoCupo: _maximoCupo, pr: 3 },
    method: "get",
    asynchronous: false,
    onComplete: function (transport) {
      var response = transport.responseText.evalJSON(true);
...
function grabarBycupoBygrupodd(_IDG, _celda, _maximoCupo) {
  var respuesta = false;
  new Ajax.Request("grupodd-1-2.php", {
    parameters: { IDG: _IDG, maximoCupo: _maximoCupo, celda: _celda },
    method: "get",
    asynchronous: false,
    onComplete: function (transport) {
      var response = transport.responseText.evalJSON(true);
...
function exeAcceso() {
  ...
  document.getElementById("idusuario").disable = "disabled";
  document.getElementById("idclave").disable = "disabled";
  document.getElementById("btaccess").style.display = "none";
  document.getElementById("wait").style.display = "";

  fetch("../login.min.ventas.php?op=1&usu=" + idusuario + "&pwd=" + hex_md5(clave) + "&ck=" + getuser)
  .then(res => res.text())
  .then(response=>{
    var arrInfo = response.split("||");
    if (!eval(arrInfo[0])) {
      document.getElementById("wait").style.display = "none";
      document.getElementById("btaccess").style.display = "";
      document.getElementById("idusuario").disable = "";
      document.getElementById("idclave").disable = "";
      document.getElementById("idusuario").value = "";
      document.getElementById("idclave").value = "";
      switch (arrInfo[1]) {
        case "1":
          document.getElementById("repuesta").innerHTML = '<div class="alert alert-danger" role="alert">Usuario Bloqueado!</div>';
          break;
        case "3":
          nalert("HORARIO", "ESTA FUERA DEL HORARIO PARA LA VENTA!");
          break;
        case "4":
          document.getElementById("repuesta").innerHTML = '<div class="alert alert-danger" role="alert">Solo acceso a clientes!</div>';
          break;
        default:
          document.getElementById("repuesta").innerHTML =
            '<div class="alert alert-danger" role="alert">El usuario o clave no es correcta!</div>';
      }
      ...
      document.getElementById("idclave").value = "";
      var Info = response.split("&");
      window.location.replace("../intro.min.ventas.php?code=" + Info[1]);
    }  
  })
  .catch(function(e){
    document.getElementById("idusuario").disable = "";
...
function loginVentas(){
  var winx = createwindow('BIENVENIDO', '', '', true, false,true,'login-windows')
  Layout(winx, '2H', ['95%', '100%'])

  const FORMACCES= "form-accces"
  const FORMREGISTRO= "form-registro"

  let accesoperm = true
  if (!isset(FORMACCES)){
    new Ajax.Request('login-access.php', {
      method: "get",
      asynchronous: false,
      onComplete: function (transport) {
				$("respose-form").innerHTML = transport.responseText ;
      },
      onFailure: function () {
        alert("No tengo respuesta Comuniquese con el Administrador!");
        accesoperm=false
  ...
  if (!accesoperm) return

  attachObjectFramer("A", FORMACCES, winx)
  const formAccces = $(FORMACCES)
  formAccces.classList.remove('d-none')

  attachObjectFramer("B", FORMREGISTRO, winx)
  const formRegistro = $(FORMREGISTRO)
  formRegistro.classList.remove("d-none")

  const USUARIO = document.getElementById('idusuario');
  const IDUSUARIO = USUARIO.id
  const IDCLAVE = document.getElementById('idclave').id;
  const form = document.getElementById('form-accces');
  form.addEventListener('keypress', (e) => {
      document.getElementById("repuesta").innerHTML = "<div></div>"
      if (e.key === 'Enter') {
          e.preventDefault();
          const target = e.target.id;
          if (target === IDUSUARIO) {
              document.getElementById(IDCLAVE).focus();
          } else if (target === IDCLAVE) {
              exeAcceso()
              USUARIO.focus();
          }
      }
...
function showWindowInfo(){
  var winxInfo = createwindow('NOTICIA', '', '65%', true, false,true,'login-info')
  Layout(winxInfo, '1H', [ '100%'])
  const FORMACCES= "form-info"
  attachObjectFramer("A", FORMACCES, winxInfo)
  const formAccces = $(FORMACCES)
  formAccces.classList.remove('d-none')
...
function execMenu(id, zoneId, casState) {
  ...
  if (a[0] == "s") {
    //
    new Ajax.Request("animalitos/vermenu.php", {
      parameters: { idmenu: id, idt: $("usu").title },
      method: "post",
      asynchronous: false,
      onComplete: function (transport) {
        var response = transport.responseText;
  ...
  if (zona[0] == "m4") {
    if (zona[0] == "m4") window.close();
  } else{
    
    const ENV = zona[0] == "mfVentasDEV"?"DEV":"PRO"

    if (zona[0] == "mfVentas" || zona[0] == "mfVentasDEV") {
      callSale2s(ENV)
    }else{
        callModule = "vermenu.ventas.php"
        console.log(id)
        thisMicroFronter = id.split("")
        mf = thisMicroFronter[0]+thisMicroFronter[1]
        console.log(mf)
        if (mf === "mf") {
          callModule = "vermenuformf.php"
         
        }

        new Ajax.Request(callModule, {
          parameters: { idmenu: id, idt: $("usu").title },
          method: "post",
          asynchronous: false,
          onComplete: function (transport) {
            var response = transport.responseText;
            if (callModule === 'vermenu.ventas.php')
               response.evalScripts();
            else
               callMicrfronter(response)
          },
          onFailure: function () {
            alert("No tengo respuesta Comuniquese con el Administrador!");
          },
...
function grabar_banca() {
  ...
  new Ajax.Request(
    "banca-1-2.php?nm=" + element1 + "&res=" + element2 + "&es=" + element5 + "&idg=" + element6 + "&pr=1",
    {
      method: "get",
      asynchronous: false,
      onComplete: function (transport) {
...
function eliminar_banca() {
  ...
  if (desci == true) {
    element1 = $("n_idg").title;
    new Ajax.Request("banca-1-2.php?idg=" + element1 + "&pr=2", {
      method: "get",
      asynchronous: false,
      onComplete: function (transport) {
        var response = transport.responseText.evalJSON(true);
        ...
        if (response) {
          ...
          makeRequestSP("banca-1-1.php");
        } else nalert("ERROR", "DISCULPE NO PUEDO ELIMINAR ESTA BANCA!!");
      },
...
function ImportarLogros() {
  ...
  if (desci == true) {
    new Ajax.Request(
      "jornadabb-1-2.php?hasta=" + $("fromcambio").lang + "&desde=" + $("desde").value + "&op=3&idt=" + $("usu").title,
      {
        method: "get",
        asynchronous: false,
        onComplete: function (transport) {
          ...
          if (response) {
            ...
            dhxWins1.window("w1").close();
          } else nalert("ERROR", "DISCULPE NO PUEDO REALIZAR LA OPERACION");
        },
        onCreate: function () {
          mientrasProceso("Jornada Grabada", "Grabando Jornada");
        },
  ...
  new Ajax.Request("procierre.php?op=21&idj=" + idj + "&grupo=" + gp, {
    ...
    onSuccess: function (transport) {
      ...
      if (response) {
        ...
        grabar_cnf1(
          2,
          "Ides.value|IDP.value|Grupo.value|Fecha.value|Hora.value|Escrute.value|IDJ.value|juegocompleto.checked",
          "_tbescrute",
          false
        );
        opmenu_escrute("escrute-1.php?idj=" + idj + "&idg=" + gp);
      } else nalert("INFORMACION", "La Publicacion de los Resultado esta Activada");
    },
...
function OpcionVerificarPublica(e) {
  var t
  new Ajax.Request('fescrutes.php?serial=' + e, {
    method: 'get',
    asynchronous: !1,
    onComplete: function (e) {
    ...
    'NE' != t[0]
      ? new Ajax.Request('procierre.php?op=4&serial=' + e, {
          method: 'get',
          asynchronous: !1,
          onCreate: function () {
          ...
          onComplete: function (n) {
            var o = n.responseText.evalJSON(!0)
            if (o[0]) {
              for (
                arl = o,
                  valoreex = arl[0].split('|'),
                  ...
                  a =
                    '<table border="0" style="font-size:12px; font-family:Arial, Helvetica, sans-serif;width:100%;line-height:18px"> <tr>   <th colspan="3" scope="col" >' +
                    tti +
                    " Ticket No: <label style='font-size:18px; font-weight: bold;'>" +
                    e +
                    '</label></th> </tr><tr style="background: #ede9e9;">   <th colspan="3" scope="col" >Fecha: ' +
                    valoreex[1] +
                    ' Hora:' +
                    valoreex[0] +
                    '</th> </tr><tr style="background: #ede9e9;" >   <th colspan="3" scope="col" >Punto de Venta: <label style=" font-weight: bold;">' +
                    conces +
                    ' ' +
                    direcc +
                    ' </label></th> </tr> <tr style="background: #ccc;font-weight: bold;">    <th width="5" scope="col" style="font-weight:bold">Hor.</th> <th  scope="col" width="78" style="font-weight:bold">Equipos</th>    <th width="55" scope="col" style="font-weight:bold">Logro</th>  </tr>',
                  i = 1;
                i <= arl.length - 2;
                i++
              ) {
                switch (((dequi = arl[i].split('|')), (Tescrute = ''), t[i - 1])) {
                  case 1:
                    Tescrute = '<span style="color: #107516;">-GANO</span>'
                    break
                  case 0:
              ...
              ;(a +=
                '<tr><th colspan="3" scope="col" style="background: #ccc;height: 10px;"></th></tr><tr><th colspan="2" scope="col"><label style="font-size: 14px;font-weight:bold;">Apuesta</label></th><th scope="col" ><label style="font-size: 14px;font-weight:bold;text-align:right;width:75%;    height: 0px;" >' +
                valoreex[5] +
                valoreex[2] +
                '</label></p></th> </tr><tr><th colspan="2" scope="col"><label >A Cobrar</label></th> <th scope="col" ><label style="font-size: 14px;text-align:right;width:75%;    height: 0px;"  >'  +
                valoreex[5] +
                valoreex[3] +
                '</label></p></th> </tr>  <tr><th colspan="2" scope="col"><label style="font-weight:bold;">Su Ganancia</label></th><th scope="col"><label style="font-size: 14px;font-weight:bold;text-align:right;width:75%;    height: 0px;"  >' +
                valoreex[5] +
                redond(parseFloat(valoreex[3]) - parseFloat(valoreex[2]), 2) +
                "</label></p></th>  </tr>"),
                ($('resultados').innerHTML = a + '</table>')
            } else $('resultados').innerHTML = '<blink>ESTE TICKET NO EXISTE</blink>'
          },
          onFailure: function () {
            alert('No tengo respuesta Comuniquese con el Administrador!')
          }
...
function CkeckEstadistica() {
  ...
  new Ajax.Request("verEstadistica.php", {
    parameters: { grupo: $("deporte").value, IDJ: $("fc").lang },
    method: "post",
    asynchronous: false,
    onComplete: function (transport) {
      var response = transport.responseText.evalJSON(true);
      if (response) makeResultwin("verestadiscaxml-2.php?grupo=" + $("deporte").value, "resul_estadistica");
    },
    onFailure: function () {
      alert("No tengo respuesta Comuniquese con el Administrador!");
    },
...
function AutoRefress(op) {
  if (op == 1) timerID1 = setInterval("Enlinea()", 300000);
  if (op == 2) clearInterval(timerID1);
}
function createByXMLLogs() {
  new Ajax.Request("xmlAuditoria_Acceso-2.php", {
    method: "get",
    asynchronous: false,
    onComplete: function (transport) {
      var response = transport.responseText;
...
function createByxmlTreePrincipal() {
  new Ajax.Request("xmlcreatertree-2.php", {
    method: "get",
    asynchronous: false,
    onComplete: function (transport) {
      var response = transport.responseText;
...
function createByxmlTreeDeportes() {
  new Ajax.Request("xmlcreatertree-3.php", {
    method: "get",
    asynchronous: false,
    onComplete: function (transport) {
      var response = transport.responseText;
...
function imprimirreddxDerecho() {
  ...
  abrir(
    "reportedeventasderecho.php?d1=" + d1 + "&d2=" + d2 + "&clsi=" + clsi1 + "&gp=" + gp,
    "Reporte de Ventas Resumido ",
    1,
    0,
    0,
...
function verEquipos(idEqui, Grupo, Liga) {
  makeResultwin(
    "jornadabb-1-8.php?Dequipo=" +
      $("equipo" + idEqui).innerHTML +
      "&Grupo=" +
      Grupo +
      "&Liga=" +
      Liga +
...
function Devolver(idEqui, Grupo, Liga) {
  if (confirm("Dese regresar el cambio del equipo?"))
    new Ajax.Request(
      "jornadabb-1-9.php?op=1&EDevolver=" +
        $("equipoDev" + idEqui).innerHTML +
        "&Grupo=" +
        Grupo +
        "&Liga=" +
        Liga +
        ...
        $("equipo" + idEqui).lang,
      {
        ...
        onSuccess: function (transport) {
          ...
          } else alert("Disculpe no puedo devolver los cambios!");
        },
        onFailure: function () {
          alert("No tengo respuesta Comuniquese con el Administrador!");
        },
...
function VerificarLogros(op) {
  ...
  element5 = cant;
  for (j = 1; j <= cant; j++)
    for (i = 0; i <= valoreslista.length - 4; i++) {
      $("ERR_" + valoreslista[i] + "_" + j).innerHTML = "";
    }
  for (j = 1; j <= cant; j++) {
    hrx += $("hora" + j).value + ":" + $("min" + j).value + "|";
    pch += $("pc1" + j).value + "|";
  ...
  for (i = 1; i <= cant; i++) {
    if ($("equipo" + i).title == "2k") equip += "$" + $("equipo" + i).innerHTML + "|";
    else equip += $("equipo" + i).value + "|";
  ...
  new Ajax.Request("jornadabb-1-10.php", {
    parameters: {
      idc: element1,
      lista: listado,
      fc: element2,
        ...
        for (i = 0; i <= response.length - 1; i++) {
          ...
          switch (codigo[0]) {
            ...
            case "6":
              ...
              if (!op)
                if (confirm("Desea grabar los Logros con este Error?")) {
                  grabar = true;
                }
              break;
            case "3":
            ...
            case "4":
              ErrorSobra(codigo[1], valorLinea, codigo[3], "#C00");
              if (codigo[4] == 1) $("ERR_" + codigo[3] + "_" + valorLinea).innerHTML = "Fuera de Rango de Logros";
              else $("ERR_" + codigo[3] + "_" + valorLinea).innerHTML = "Fuera de Rango de Carreraje/Puntos";
              break;
          }
        }
    ...
    onCreate: function () {
      mientrasProceso("Chequeando  Logros", "Jornada Validada");
    },
    onFailure: function () {
      alert("No tengo respuesta Comuniquese con el Administrador!");
    },
...
function ErrorSobra(Logros, Linea, Columna, Adver) {
  ...
  for (u = 0; u <= iverLogros.length - 1; u++) {
    ...
    if (verGuion.length == 2) campo = verGuion[1];
    else campo = verGuion[0];
...
function createXMLOdds(Grupo) {
  new Ajax.Request("chkOddsTree.php", {
...
function chkOddOpt(op) {
  for (u = 1; u <= Camposl.length - 1; u++) {
    $("tx" + Camposl[u]).style.display = "none";
    $("cp" + Camposl[u]).style.display = "none";
  }

  $("tx" + Camposl[op]).style.display = "";
  $("cp" + Camposl[op]).style.display = "";
}
...
function GrabarchkOdds() {
  $("from1").request({
    ...
    onComplete: function (transport) {
      ...
      } else nalert("ERROR", "Disculpe: pero la informacion no se almaceno!");
    },
    onFailure: function () {
      alert("No tengo respuesta Comuniquese con el Administrador!");
    },
...
function EliminarchkOdds() {
  new Ajax.Request("chkOdds-3.php", {
    parameters: { newi: $("new").value },
    method: "get",
    asynchronous: false,
    onComplete: function (transport) {
      var response = transport.responseText.evalJSON(true);
      if (response) {
        ...
        $("config").innerHTML = "";
        mygrid1.clearAndLoad("chkOdds-4.php?Grupo=" + Grupo);
      } else nalert("ERROR", "Disculpe: pero la informacion no se Elimino!");
    },
    onFailure: function () {
      alert("No tengo respuesta Comuniquese con el Administrador!");
    },
...
function checkAduiVentas() {
  mientrasProceso("Analizando", "Procesando");
  new Ajax.Request("Auditoriadeventas.php", {
    parameters: { grupo: $("deporte").value, IDJ: $("fc").lang },
    method: "post",
    asynchronous: false,
    onComplete: function (transport) {
      var response = transport.responseText.evalJSON(true);
      if (response) makeResultwin("Auditoriadeventas-2.php?grupo=" + $("deporte").value, "resul_Venta");
    },
    onFailure: function () {
      alert("No tengo respuesta Comuniquese con el Administrador!");
    },
...
function SeleMonto(i) {
  switch (i) {
    case 1:
      $("Monto_TOtal").disabled = "disabled";
      break;
    case 2:
      $("Monto_TOtal").disabled = "";
      $("Monto_TOtal").select();
      $("Monto_TOtal").focus();
      break;
  }
...
function iSumaValores() {
  if ($("iEfectivo").value == "") valor1 = 0;
  else valor1 = parseFloat($("iEfectivo").value);
  if ($("iCheque").value == "") valor2 = 0;
  else valor2 = parseFloat($("iCheque").value);
  if ($("iTransfer").value == "") valor3 = 0;
  else valor3 = parseFloat($("iTransfer").value);
...
///////////////// Function Set Escrute ////////////////
function set_procces_save_Data(IDP, activo, focusfirst, asyn) {
  const result = set_ActiveElement(IDP, false, focusfirst, false)
  const stateSave = save_score(IDP, result, activo, focusfirst, asyn)
  return stateSave
}

function save_score(IDP, data, activo, focusfirst, asyn) {
  let state_Retur = true
  new Ajax.Request('procierre.php', {
    ...
    onComplete: function(r) {
      ...
      if (d.state) {
        set_ActiveElement(IDP, true, focusfirst, !activo)
        return
      } else {
        if (!d.closeplay) {
          alert('El Partido no esta Cerrado!! (Debe cerrarlo para colocar los resultados)');
          state_Retur = false
          return
        }
        if (d.publicresul) {
          alert('Los Resultados estan Publicados!! (No puedo aceptar los nuevos resultados)');
          state_Retur = false
          return
        }
        alert('Hubo un error en grabar los datos (Code 4)');
        state_Retur = false
...
function close_play(data) {
  const [IDP, Grupo, IDJ] = data.split('-')
  const idu = $('fc').dataset.idt
  new Ajax.Request('procierre.php', {
    parameters: {
      op: 1,
      idj: IDJ,
    ...
    onComplete: function(r) {
      let d = JSON.parse(r.responseText)
      if (d) {
        set_createElementToggle(IDP)
        set_ActiveElement(IDP, true, true, false)
      } else {
        alert('No pude cerrar el Partido!!')
      }
    },
    onFailure: function() {
      alert('No tengo respuesta Comuniquese con el Administrador!')
...
function handle_set_despublic(IDJ, IDG, tg) {
  new Ajax.Request('procierre.php?op=202&idj=' + IDJ + '&grupo=' + IDG, {
    method: 'get',
    asynchronous: false,
    onComplete: function(e) {
      try {
        ...
        set_despublic(IDG, tg)
      } catch (error) {
        console.log(error)
        alert('No pude DES-PUBLICAR este deporte!! Error en el SERVIDOR')
      }
...
function set_despublic(IDG, e) {
  const frmdata = [...document.getElementById('fmr-data').children]
  const ELEMENT_TR = 'TR'
  const ELEMENT_TH = 'TH'
  const ELEMENT_DIV = 'DIV'
  frmdata.forEach(o => {
    const tg = o
    if (tg.nodeName === ELEMENT_TR && tg.id !== "") {
      const child = [...tg.children]
      child.forEach(o => {
        console.log("o", o)
        if (o.nodeName === ELEMENT_TH && o.id === "") {
          const child1 = [...o.children]
          child1.forEach(o => {
            myObj = o.children[0]
            myObj.disabled = false
          })
        } else {
          const child2 = [...o.children]
          child2.forEach(o => {
            if (o.nodeName === ELEMENT_DIV && o.id !== "") {
              const ida = 'a-' + o.id
              $(ida).disabled = false
            }
          })
  ...
  const idtable = document.getElementById(IDG)
  idtable.classList.remove('escrutin-desactive-all')
  e.remove()
}

function set_ActiveElement(IDP, setDisable, focusFirts, state) {
  ...
  const inputFirst = []
  const ELEMENT_TH = 'TH'
  const elementsObj = document.getElementById('la' + IDP).children
  const elemObs = [...elementsObj]
  elemObs.forEach(o => {
    const tg = o
    if (tg.nodeName === ELEMENT_TH && tg.id === "") {
      const child = [...tg.children]
      child.forEach(o => {
        myObj = o.children[0]
        if (myObj.nodeName === 'INPUT') {
          if (inputFirst.length === 0)
            inputFirst.push(myObj.id)
        }
        if (setDisable) {
          myObj.disabled = state
        } else {
          const [id, position] = myObj.dataset.ft.split("-")
          let idx = listElements.findIndex(o => Number(o) === Number(id))
          if (idx === -1) {
            idx = listElements.length;
            listElements.push(id)
          }
          ++idx;
          let data = listElements[idx];
          const resul = data ? data.split('-') : []
          const type = myObj.type
          if (type === INPUT_TYPE) {
            resul[position - 1] = myObj.value === "" ? "0" : myObj.value
          }
          if (type === RADIO_TYPE || type === CHECK_TYPE) {
            resul[position - 1] = Number(myObj.checked)
          }
          listElements[idx] = resul.join('-')
        }
...
function set_createElementToggle(IDP) {
  const TYPE_ELEMENT = 's'
  const CHECKBOX = "checkbox"
  const id = 'btClose-' + IDP
  const idElement = document.getElementById(id)
  const elem = [...idElement.children]
  elem.forEach(o => {
    o.remove()
  })
  const sc_label = document.createElement('label')
  sc_label.classList.add('switch')
  const sc_input = document.createElement('input')
  sc_input.id = 'a-' + id
  sc_input.type = CHECKBOX
  sc_input.dataset.type = TYPE_ELEMENT
  sc_input.dataset.idp = IDP
  sc_input.checked = true
  const sc_span = document.createElement('span')
  sc_span.classList.add('slider', 'round')

  sc_label.appendChild(sc_input)
  sc_label.appendChild(sc_span)
  idElement.appendChild(sc_label)
}

function set_focus_element(rrIdx) {
  let rIdx = rrIdx;
  let pase = 0;
  for (;;) {
    rIdx = rIdx > listobjs.length ? 0 : rIdx
    if (pase === 2) break
    const nextId = listobjs[rIdx]
    if (isset(nextId)) {
      const state = $(nextId).disabled
      if (!state) {
        $(nextId).focus()
        $(nextId).select()
        break;
