...
function dhtmlXTabBar(parentObject, mode, height) {
  ...
  if (typeof parentObject != 'object') parentObject = document.getElementById(parentObject)
...
dhtmlXTabBar.prototype = {
  _get_size: function (name, alter) {
    var size =
      this.entBox.getAttribute(name) ||
      this.entBox.style[name] ||
      (window.getComputedStyle
        ? window.getComputedStyle(this.entBox, null)[name]
        : this.entBox.currentStyle
        ? this.entBox.currentStyle[name]
        : 0)
  ...
  _checkScroll: function () {
    ...
    for (var i = 0; i < this._rows.length; i++)
      if (this._rows[i].scrollWidth - this._rows[i].offsetWidth > 2) this._showScroll(i)
  ...
  _setRowSizes: function () {
    for (var i = 0; i < this._rows.length; i++) {
      ...
      this._rows[i].style[this._py] =
        ...
        (_isIE && !window.postMessage && this._s._bMode ? 2 : 0) +
        'px'
  ...
  removeTab: function (id, mode) {
    ...
    row.removeChild(tab)
    row.tabCount--
    if (row.tabCount == 0 && this._rows.length > 1) this._removeRow(row)
  ...
  _img: function (tag, y, pos, a, b) {
    tag.style.backgroundImage =
      'url(' + this.imgUrl + this.skin + '/' + tag.parentNode.skin + '_' + this._s.mode + '.gif)'
  ...
  _showHover: function (tab) {
    ...
    if (this._lastHower && this._lastHower != this._lastActive) {
      ...
      if (a.hover) this._img_all(this._lastHower)
      else
        this._img(
          this._lastHower.childNodes[3],
          5,
          this._px,
          parseInt(this._lastHower.style[this._dx]) - (a.left + a.right),
          a.left
        )
  ...
  _onClickHandler: function (e) {
    ...
    if (target && !target._disabled) this.tabbar._setTabActive(target)
    else {
      var tag = null
  ...
  _setTabTop: function (tab) {
    ...
    for (var i = 0; i < this._rows.length; i++)
      if (this._rows[i] == tab.parentNode) {
        ...
        var wh = this._s._vMode
          ? Math.max(this._tabZone.offsetHeight, row.scrollHeight)
          : Math.max(this._tabZone.offsetWidth, row.scrollWidth)
    ...
    if (!this._content[id]) {
      ...
      d.show = function () {
        ...
        if (self._awdj || self._ahdj) {
          ...
          if (self._awdj) self.entBox.style.width = dim[0] + 2 + 'px'
          if (self._ahdj) self.entBox.style.height = dim[1] + self._rows.length * (self._s.line_height - 2) + 2 + 'px'
          self._setSizes()
      ...
      d.hide = function () {
        ...
        this.style.zIndex = -1
        if (this._hrfmode == 'iframe') this.attachURL('javascript:false')
      }
      d.activate = function () {
        ...
        if (this._hrfmode != 'iframe') this._delay = null
      }
  ...
  forceLoad: function (id, href) {
    if (href) this.setContentHref(id, href)
  ...
  _parseXML: function (that, a, b, c, obj) {
    ...
    for (var i = 0; i < arows.length; i++) {
      ...
      for (var j = 0; j < atabs.length; j++) {
        ...
        if (that._hrfmode) that.setContentHref(id, atabs[j].getAttribute('href'))
        else
          for (var k = 0; k < atabs[j].childNodes.length; k++) {
            var cont = atabs[j].childNodes[k]
            if (cont.tagName == 'content') {
              if (cont.getAttribute('id')) that.setContent(id, cont.getAttribute('id'))
              else that.setContentHTML(id, that._getXMLContent(cont))
            }
          }
      }
    }
  ...
  setCustomStyle: function (id, color, scolor, style) {
    ...
    if (this._tabs[id])
      this._tabs[id].childNodes[0].style.cssText =
        this._tabs[id] == this._lastActive ? this._c[id].scolor : this._c[id].color
  },
  ...
  hideTab: function (id) {
    ...
    if (tab == this._lastActive) this.goToNextTab()
    this._setTabSizes(tab.parentNode)
  },
  ...
  normalize: function (limit, full) {
    ...
    for (var j = 0; j < this._rows.length; j++)
      for (var i = 0; i < this._rows[j].tabCount; i++) tabs.push(this._rows[j].removeChild(this._rows[j].childNodes[0]))
