...
function dhtmlxCalendarObject(base, isAutoDraw, options) {
  ...
  if (options) for (param in options) this.options[param] = options[param]
  this.allYears = Array()
  with (this.options) for (var i = yearsRange[0]; i <= yearsRange[1]; i++) this.allYears.push(i)
...
dhtmlxCalendarObject.prototype = {
  createStructure: function () {
    ...
    if (this.con[0].nodeName == 'INPUT') {
      ...
      conOnclick = function (e) {
        ...
        if (this.id != self.activeCon.id) self.show(this.id)
        e = e || event
  ...
  drawHeader: function () {
    ...
    var self = this
    while (this.winHeader.hasChildNodes()) this.winHeader.removeChild(this.winHeader.firstChild)
      ...
      for (var i = 0; i < 7; i++) {
        ...
        if (this.insensitiveDates) {
          ...
          for (var j = 0; j < this.insensitiveDates.length; j++) {
            ...
            if (
              (s && this.getFormatedDate(f, tempDate) == this.insensitiveDates[j]) ||
              tempDate.getDay() == this.insensitiveDates[j]
            ) {
        ...
        if (
          (this.sensitiveFrom && tempDate.valueOf() < this.sensitiveFrom.valueOf()) ||
  ...
  setDate: function (date, conId) {
    ...
    if (isNaN(date)) date = new Date(tmpDate)
    if (!isNaN(date)) {
    ...
    if (this.activeCon.nodeName == 'INPUT')
      this.activeCon.value =
        this.getFormatedDate(this.options.dateformat, date) + (this.time ? ' ' + this.getFormatedTime() : '')
  },
      ...
      if (plain) {
        ...
        switch (replStr) {
          ...
          case 'j':
            ...
            while (replStr.toString().length < 3) replStr = '0' + replStr
            break
          case 'a':
  ...
  _activeConInd: function (ind) {
    if (!this.parent) this.createStructure()
    return (this.activeConInd =
      (this.conInd[ind] == 0 ? '0' : this.conInd[ind]) || (ind == 0 ? '0' : ind) || this.conInd[this.activeCon.id] || 0)
  }
}
...
dhtmlxCalendarObject.prototype.onYearSelect = function (value) {
  if (!isNaN(value)) {
    this._c.date[this._c._activeConInd()].setFullYear(
      Math.min(Math.max(value, this._c.allYears[0]), this._c.allYears.slice(-1))
    )
  }
  this._c.draw()
  return !isNaN(value)
