...
if (!class_exists('TCPDF', false)) {
	...
	*/
	class TCPDF {
		...
		* @access protected
		*/
		protected $GLOBALS['link']s = array();
		...
		* @param int $GLOBALS['link'] The link identifier returned by AddLink()
		...
		* @see AddLink()
		*/
		public function SetLink($GLOBALS['link'], $y=0, $page=-1) {
			if ($y == -1) {
				$y = $this->y;
			}
			if ($page == -1) {
			...
			$this->links[$GLOBALS['link']] = array($page, $y);
		}
		...
		* @param mixed $GLOBALS['link'] URL or identifier returned by AddLink()
		* @param int $spaces number of spaces on the text to link
		* @access public
		* @since 1.5
		* @see AddLink(), Annotation(), Cell(), Write(), Image()
		*/
		public function Link($x, $y, $w, $h, $GLOBALS['link'], $spaces=0) {
			$this->Annotation($x, $y, $w, $h, $GLOBALS['link'], array('Subtype'=>'Link'), $spaces);
		}
		...
		*/
		public function Cell($w, $h=0, $txt='', $border=0, $ln=0, $align='', $fill=0, $GLOBALS['link']='', $stretch=0, $ignore_min_height=false) {
			...
			$this->_out($this->getCellCode($w, $h, $txt, $border, $ln, $align, $fill, $GLOBALS['link'], $stretch, $ignore_min_height));
		}
		...
		* @see Cell()
		*/
		protected function getCellCode($w, $h=0, $txt='', $border=0, $ln=0, $align='', $fill=0, $GLOBALS['link']='', $stretch=0, $ignore_min_height=false) {
			...
			if ($txt != '') {
				...
				if ($GLOBALS['link']) {
					$this->Link($xdx, $this->y + (($h - $this->FontSize)/2), $width, $this->FontSize, $GLOBALS['link'], $ns);
				}
			}
			// output cell
			if ($s) {
		...
		* @since 1.5
		*/
		public function Write($h, $txt, $GLOBALS['link']='', $fill=0, $align='', $ln=false, $stretch=0, $firstline=false, $firstblock=false, $maxh=0) {
			if (strlen($txt) == 0) {
				$txt = ' ';
			}
			// remove carriage returns
			...
			while ($i < $nb) {
				...
				if ($c == 10) { // 10 = "\n" = new line
					...
					$this->Cell($w, $h, $tmpstr, 0, 1, $talign, $fill, $GLOBALS['link'], $stretch);
					unset($tmpstr);
					if ($firstline) {
					...
					if (($l > $wmax) OR ($shy AND (($l + $tmp_shy_replacement_width) > $wmax)) ) {
						...
						if ($sep == -1) {
							...
							if (($this->rtl AND ($this->x <= ($this->w - $this->rMargin - $chrwidth)))
								...
								$this->Cell($w, $h, $tmpstr, 0, 1, $align, $fill, $GLOBALS['link'], $stretch);
								unset($tmpstr);
								if ($firstline) {
									$this->cMargin = $tmpcmargin;
									return ($this->UniArrSubString($uchars, $i));
							...
							$this->Cell($w, $h, $shy_char_left.$tmpstr.$shy_char_right, 0, 1, $align, $fill, $GLOBALS['link'], $stretch);
							unset($tmpstr);
							if ($firstline) {
			...
			if ($l > 0) {
				...
				$this->Cell($w, $h, $tmpstr, 0, $ln, $align, $fill, $GLOBALS['link'], $stretch);
				unset($tmpstr);
				if ($firstline) {
					$this->cMargin = $tmpcmargin;
		...
		* @since 1.1
		*/
		public function Image($file, $x='', $y='', $w=0, $h=0, $type='', $GLOBALS['link']='', $align='', $resize=false, $dpi=300, $palign='', $ismask=false, $imgmask=false, $border=0, $fitbox=false, $hidden=false) {
			if ($x === '') {
				$x = $this->x;
			}
			if ($y === '') {
			...
			if ($newimage) {
				...
				if ((method_exists($this, $mtd)) AND (!($resize AND function_exists($gdfunction)))) {
					...
					$info = $this->$mtd($file);
					if ($info == 'pngalpha') {
						return $this->ImagePngAlpha($file, $x, $y, $w, $h, 'PNG', $GLOBALS['link'], $align, $resize, $dpi, $palign);
					}
				} 
				if (!$info) {
			...
			if ($GLOBALS['link']) {
				$this->Link($ximg, $y, $w, $h, $GLOBALS['link'], 0);
			}
		...
		* @see Image()
		*/
		protected function ImagePngAlpha($file, $x='', $y='', $w=0, $h=0, $type='', $GLOBALS['link']='', $align='', $resize=false, $dpi=300, $palign='') {
			// get image size
			...
			$this->Image($tempfile_plain, $x, $y, $w, $h, $type, $GLOBALS['link'], $align, $resize, $dpi, $palign, false, $imgmask);
			// remove temp files
		...
		* @access public
		*/
		public function ImageEps($file, $x='', $y='', $w=0, $h=0, $GLOBALS['link']='', $useBoundingBox=true, $align='', $palign='', $border=0) {
			if ($x === '') {
				$x = $this->x;
			}
			if ($y === '') {
	...
	Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
		...
		*/
		public function movePage($frompage, $topage) {
			...
			// adjust links
			$tmplinks = $this->links;
			foreach ($tmplinks as $key => $GLOBALS['link']) {
				if (($GLOBALS['link'][0] >= $topage) AND ($GLOBALS['link'][0] < $frompage)) {
					$this->links[$key][0] = $GLOBALS['link'][0] + 1;
				} elseif ($GLOBALS['link'][0] == $frompage) {
		...
		*/
		public function deletePage($page) {
			...
			$tmplinks = $this->links;
			foreach ($tmplinks as $key => $GLOBALS['link']) {
				if ($GLOBALS['link'][0] > $page) {
					$this->links[$key][0] = $GLOBALS['link'][0] - 1;
				} elseif ($GLOBALS['link'][0] == $page) {
		...
		*/
		public function addTOC($page='', $numbersfont='', $filler='.', $toc_name='TOC') {
			...
			foreach ($this->outlines as $key => $outline) {
				...
				$GLOBALS['link'] = $this->AddLink();
				$this->SetLink($GLOBALS['link'], 0, $outline['p']);
				// write the text
				$this->Write(0, $outline['t'], $GLOBALS['link'], 0, $aligntext, false, 0, false, false, 0);
				...
				// write the number
				$this->Cell($tw, 0, $pagenum, 0, 1, $alignnum, 0, $GLOBALS['link'], 0);
				$this->SetX($x_start);
