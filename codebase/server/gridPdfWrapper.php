...
class TCPDFExt extends TCPDF {
	...
	public function Footer() {				
		...
		if ($this->getRTL()) {
			...
			$this->Cell(0, 0,'Fecha de Impresion:'.date('d/m/Y  h:i:s a').' '.$pagenumtxt, 0, 0, 'L');
			
		} else {
			$this->SetX($ormargins['left']);
			$this->Cell(0, 0,'Fecha de Impresion:'.date('d/m/Y  h:i:s a').' '.$pagenumtxt, 0, 0, 'R');
		}
	}
...
class gridPdfWrapper {
	...
	// draws grid header
	public function headerDraw($headerHeight, $columns, $summaryWidth, $headerTextColor, $bgColor, $lineColor,$f1,$f2) {
        ...
        $this->cb->Cell(0, 0,'Fecha Desde:'.$f1.' Hasta:'.$f2, 0, 0, 'L');
	...
	public function gridDraw($rowHeight, $rows, $widths, $startRow, $rowsNum, $scaleOneColor, $scaleTwoColor, $profile) {
		...
		$printedRowsNum = 0;
		$last = '0';
		$limitY = $this->cb->getPageHeight() - $this->offsetBottom;
		while (($this->currentRow < count($this->rows))&&($this->cb->getY() + $this->getMaxRowHeight($this->currentRow) <= $limitY)) {
			// selection color of row
			if ($this->currentRow%2 == 0) {
				$this->cb->SetFillColor($this->scaleOneColor['R'], $this->scaleOneColor['G'], $this->scaleOneColor['B']);
			} else {
				$this->cb->SetFillColor($this->scaleTwoColor['R'], $this->scaleTwoColor['G'], $this->scaleTwoColor['B']);
			}
			...
			for ($j = 0; $j < count($this->rows[$this->currentRow]); $j++) {
				...
				$xImg = $this->cb->getX();
				$yImg = $this->cb->getY();
				$value = $this->rows[$this->currentRow][$j];
				...
				if (($this->columns[0][$j]['type'] == 'ch')||($this->columns[0][$j]['type'] == 'ra')) {
					$xImg += $width/2 - 1.5;
					$yImg += 1.7;
					$img = $this->getImg($this->columns[0][$j]['type'], $this->rows[$this->currentRow][$j], $this->profile);
				...
				if (($this->columns[0][$j]['type'] == 'img')&&((strpos($this->rows[$this->currentRow][$j], 'http://') === 0)||(file_exists($this->rows[$this->currentRow][$j])))) {
					$imgSize = getimagesize($this->rows[$this->currentRow][$j]);
					...
					$this->cb->Image($this->rows[$this->currentRow][$j], $xImg, $yImg + 0.5, $widthImg, $heightImg, '', '', 'M', false, $this->dpi, 'L', false, false, 0, false, false);
					$this->cb->setX($x);
					$this->cb->setY($y, false);
				}
			}
	...
	private function getMaxRowHeight($num) {
		...
		for ($j = 0; $j < count($this->rows[$num]); $j++) {
			...
			if ($this->columns[0][$j]['type'] == 'img') {
				// if file exists
				if ((strpos($this->rows[$this->currentRow][$j], 'http://') === 0)||(file_exists($this->rows[$this->currentRow][$j]))) {
					$imgSize = getimagesize($this->rows[$num][$j]);
				...
				$linesNum = $this->cb->getNumLines($this->rows[$num][$j],$width);
	...
	// draws footer image
	public function drawImgFooter($filename) {
