						...
						if (!$this->_readDataOnly) {
							...
							if ($xmlSheet && $xmlSheet->hyperlinks) {
								foreach ($xmlSheet->hyperlinks->hyperlink as $hyperlink) {
									// Link url
									$GLOBALS['link']Rel = $hyperlink->attributes('http://schemas.openxmlformats.org/officeDocument/2006/relationships');

									foreach (PHPExcel_Cell::extractAllCellReferencesInRange($hyperlink['ref']) as $cellReference) {
										if (isset($GLOBALS['link']Rel['id'])) {
											$docSheet->getCell( $cellReference )->getHyperlink()->setUrl( $hyperlinks[ (string)$GLOBALS['link']Rel['id'] ] );
										}
