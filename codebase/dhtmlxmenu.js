...
function dhtmlXMenuObject(baseId, skin) {
  ...
  this.setSkin = function (skin) {
    ...
    if (this.base._autoSkinUpdate) {
      this.base.className =
        this.base.className.replace('dhtmlxMenu_' + oldSkin + '_Middle', '') + ' dhtmlxMenu_' + this.skin + '_Middle'
    }
  }
  ...
  this._redistribSubLevelSelection = function (id, parentId) {
    ...
    if (this.itemPull[id]['state'] == 'enabled') {
      ...
      if (this.itemPull[id]['complex'] && this.dLoad && this.itemPull[id]['loaded'] == 'no') {
        ...
        xmlLoader.loadXML(
          this.dLoadUrl + '?action=loadMenu&parentId=' + id.replace(this.idPrefix, '') + '&etc=' + new Date().getTime()
        )
      }
  ...
  this._buildMenu = function (t, parentId) {
    ...
    for (var q = 0; q < t.childNodes.length; q++) {
      if (t.childNodes[q].tagName == this.itemTagName) {
        ...
        item['state'] =
          r.getAttribute('enabled') != null
            ? r.getAttribute('enabled') == 'false'
              ? 'disabled'
              : 'enabled'
            : 'enabled'
        item['parent'] = parentId != null ? parentId : this.idPrefix + this.topId
        item['complex'] = this.dLoad
          ? r.getAttribute('complex') != null
            ? true
            : false
        ...
        for (var w = 0; w < r.childNodes.length; w++) {
          ...
          if (tagNm == this.userDataTagName) {
            ...
            if (d.getAttribute('name') != null) {
              this.userData[item['id'] + '_' + d.getAttribute('name')] =
                d.firstChild.nodeValue != null ? d.firstChild.nodeValue : ''
            }
          }
    ...
    if (main_self.dLoad) {
      ...
      if (parentId == null) {
        ...
        if (main_self.menuSelected == main_self.idPrefix + parentId) {
          ...
          var isTop =
            main_self.itemPull[main_self.idPrefix + parentId]['parent'] == main_self.idPrefix + main_self.topId
...
dhtmlXMenuObject.prototype.init = function () {
  ...
  if (this.dLoad) {
    this.callEvent('onXLS', [])
    this._xmlLoader.loadXML(this.dLoadUrl + '?action=loadMenu&etc=' + new Date().getTime())
  } else {
  ...
  if (this.itemPull[id]['complex']) {
    ...
    if (this.dLoad && this.itemPull[id]['loaded'] == 'get' && this.loaderIcon) {
      ...
      arrowObj.src =
        this.skinPath +
        'dhtmlxmenu_sub' +
        (this.itemPull[id]['state'] == 'enabled' ? (over ? 'over' : 'enable') : 'disable') +
        '.gif'
    }
    return
  }
...
dhtmlXMenuObject.prototype._changeItemState = function (id, newState, levelType) {
  ...
  if (this.itemPull[j] != null && this.idPull[j] != null) {
    if (this.itemPull[j]['state'] != newState) {
      ...
      this.idPull[j].className =
        'dhtmlxMenu_' +
        this.skin +
        '_' +
        (!this.context ? levelType : 'SubLevelArea') +
        '_Item_' +
...
dhtmlXMenuObject.prototype._addUpArrow = function (id) {
  ...
  arrow.onmouseout = function () {
    if (main_self.menuMode == 'web') {
      window.clearTimeout(main_self.menuTimeoutHandler)
      main_self.menuTimeoutHandler = window.setTimeout(
        function () {
          main_self._clearAndHide()
        },
        main_self.menuTimeoutMsec,
        'JavaScript'
      )
    }
    this.over = false
...
dhtmlXMenuObject.prototype._updateCheckboxImage = function (id) {
  ...
  this.itemPull[this.idPrefix + id]['imgdis'] =
    'dhtmlxmenu_chbxdis_' + (this._getCheckboxState(id) ? '1' : '0') + '.gif'
  this.idPull[this.idPrefix + id].childNodes[0].src =
    this.skinPath +
    (this.itemPull[this.idPrefix + id]['state'] == 'enabled'
      ? this.itemPull[this.idPrefix + id]['imgen']
      : this.itemPull[this.idPrefix + id]['imgdis'])
}
...
dhtmlXMenuObject.prototype._updateItemImage = function (id, levelType) {
  ...
  if (this.itemPull[this.idPrefix + id]['type'] == 'radio') {
    var imgSrc =
      this.itemPull[this.idPrefix + id][this.itemPull[this.idPrefix + id]['state'] == 'enabled' ? 'imgen' : 'imgdis']
  } else {
    var imgSrc =
      this.itemPull[this.idPrefix + id][this.itemPull[this.idPrefix + id]['state'] == 'enabled' ? 'imgen' : 'imgdis']
  }
  if (imgSrc.length > 0) {
    if (imgObj != null) {
      imgObj.src =
        (this.itemPull[this.idPrefix + id]['type'] == 'checkbox' || this.itemPull[this.idPrefix + id]['type'] == 'radio'
          ? this.skinPath
      ...
      if (levelType == 'TopLevel') {
        this.idPull[this.idPrefix + id].style.paddingLeft = this.topLevelItemPaddingIconExists + 'px'
      }
    }
  } else {
    if (imgObj != null) {
...
dhtmlXMenuObject.prototype._updateLoaderIcon = function (id, state) {
  ...
  for (var q = 0; q < this.idPull[id].childNodes.length; q++) {
    if (this.idPull[id].childNodes[q].tagName == 'IMG') {
      if (
        this.idPull[id].childNodes[q].className ==
        'dhtmlxMenu_' + this.skin + '_SubLevelArea_Item_Arrow' + (state ? '' : '_Loading')
      ) {
        this.idPull[id].childNodes[q].className =
          'dhtmlxMenu_' + this.skin + '_SubLevelArea_Item_Arrow' + (state ? '_Loading' : '')
...
dhtmlXMenuObject.prototype._addItemIntoGlobalStrorage = function (
  itemId,
  itemParentId,
  itemText,
  itemType,
...
dhtmlXMenuObject.prototype._renderToplevelItem = function (id, pos) {
  ...
  m.onmouseover = function () {
    ...
    if (main_self.itemPull[this.id]['state'] == 'enabled') {
      ...
      main_self.menuSelected =
        main_self.menuMode == 'win' ? (main_self.menuSelected != -1 ? this.id : main_self.menuSelected) : this.id
      if (main_self.dLoad && main_self.itemPull[this.id]['loaded'] == 'no') {
        ...
        xmlLoader.loadXML(
          main_self.dLoadUrl +
            ...
            this.id.replace(main_self.idPrefix, '') +
            '&etc=' +
            new Date().getTime()
        )
      }
  ...
  m.onmouseout = function () {
    if (
      !(main_self.itemPull[this.id]['complex'] && main_self.menuSelected != -1) &&
...
dhtmlXMenuObject.prototype._renderSublevelItem = function (id, pos) {
  ...
  var imgPath =
    this.itemPull[id]['type'] == 'checkbox' || this.itemPull[id]['type'] == 'radio' ? this.skinPath : this.imagePath
  if (this.itemPull[id]['complex']) {
    var j_nodes =
      "<img src='" +
      this.skinPath +
      ...
      this.skin +
      "_SubLevelArea_Item_Arrow' id='arrow_" +
      this.itemPull[id]['id'] +
      "'>"
  } else {
    var j_nodes = ''
...
dhtmlXMenuObject.prototype.setHotKey = function (id, hkey) {
  ...
  for (var q = 0; q < this.idPull[id].childNodes.length; q++) {
    if (
      this.idPull[id].childNodes[q].tagName == 'DIV' &&
      this.idPull[id].childNodes[q].className == 'dhtmlxMenu_' + this.skin + '_SubLevelArea_Item_HotKey'
    ) {
