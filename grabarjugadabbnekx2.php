										...
										if (verificaciondejugada($horaticket, str_replace(')', '%', $jgd), $idj, $fechaactual, $idc)) : // <== Si No Hay Partidos Cerrados ////
											...
											if ($veri[0]) : ////$arr[0]=false; $arr[1]=$row['Minimas']; $arr[2]=$rowG['Descripcion']; $arr[3]=1;
												...
												if ($veri2[0]) :
													...
													$result2 = mysqli_query($GLOBALS['link'], "INSERT INTO _tjugadabb2  VALUES (" . $serial . "," . $idj . ",'" . str_replace(')', '%', $jgd) . "'," . $ap . "," . $aco . ",'" . $se . "','" . $idc . "','" . $horaticket . "'," . $terminal . ",''," . $usu . "," . $grp . ",1,0,'','" . $ip . "',0,'',''," . $idm . ")");
													$result = false;
													$result = mysqli_query($GLOBALS['link'], "INSERT INTO _tjugadabb  VALUES (" . $serial . "," . $idj . ",'" . str_replace(')', '%', $jgd) . "'," . $ap . "," . $aco . ",'" . $se . "','" . $idc . "','" . $horaticket . "'," . $terminal . ",''," . $usu . "," . $grp . ",1,0,'','" . $ip . "',0,'',''," . $idm . ")");
...
function verificaciondejugada($horaticket, $jugada, $idj, $fecha, $idc)
{
	...
	if ($IDJReal == $idj) :
		...
		for ($u = 0; $u <= count($jgdad) - 2; $u++) {
			...
			//   echo "Select * From _partidosbb Where (IDE1=".$equi." or IDE2=".$equi.") and IDJ=".$idj; echo '<br>';
			$result2 = mysqli_query($GLOBALS['link'], "Select * From _partidosbb Where (IDE1=" . $equi . " or IDE2=" . $equi . ") and IDJ=" . $idj);
			$row2 = mysqli_fetch_array($result2);
			//   echo$row2['Hora'].'-'.$hora;echo '<br>';
...
function verificaciondelogros($jugada, $IDJ, $IDC)
{
	...
	for ($k = 0; $k <= (count($jgdad) - 2); $k++) {
		$check = false;
		$opcion = explode('|', $jgdad[$k]);
		$logro = intval(str_replace("+", "", $opcion[1])); //$opcion[1];
		$oldLogro = $logro;
		//   echo $logro.'<br>';
		$opcion1 = explode('%', $opcion[0]);
		$carr = $opcion1[1];
