...
class PDF extends FPDF
{
	...
	function Header()
	{
		...
		$this->text(10, 7, 'REPORTE SEMANAL');
...
if (mysqli_num_rows($result) != 0) :
	...
	$aat[$i + 3] = 0;
	$bbt[$i + 3] = 0;
	$result = mysqli_query($GLOBALS['link'], "SELECT _tjugadahi.*,_tconsecionario.IDG,_tconfighi._Fecha  FROM _tjugadahi,_tconsecionario,_tconfighi  where _tjugadahi.IDC=_tconsecionario.IDC    and  _tjugadahi.IDCN=_tconfighi.IDCN  and  (anulado=0 or anulado=4 or anulado=3 )   " . $add . " order by IDC,IDJug,IDG,_tjugadahi.IDCN,carr,serial ");

	//echo ("SELECT _tjugadahi.*,_tconsecionario.IDG,_tconfighi._Fecha  FROM _tjugadahi,_tconsecionario,_tconfighi  where _tjugadahi.IDC=_tconsecionario.IDC    and  _tjugadahi.IDCN=_tconfighi.IDCN and _tconsecionario.estatus=1 and  (anulado=0 or anulado=4 or anulado=3 )   ".$add." order by IDC,IDJug,IDG,_tjugadahi.IDCN,carr,serial " );
