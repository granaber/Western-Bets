...
class PDF extends FPDF
{
	...
	function Header()
	{
		...
		$this->text(10, 7, 'REPORTE GENERAL  POR CONCESIONARIO');
...
if (mysqli_num_rows($result) != 0) :
	...
	if ($grupo != 'T') :
		$add = $add . " and _tconsecionario.IDG=" . $grupo;
		$ttex3 = " Grupo: " . $grupo;
	endif;
	...
	$result = mysqli_query($GLOBALS['link'], "SELECT _tjugadahi.*,_tconsecionario.IDG  FROM _tjugadahi,_tconsecionario  where _tjugadahi.IDC=_tconsecionario.IDC    and  _tconsecionario.estatus=1 and   (anulado=0 or anulado=4 or anulado=3 )   " . $add . " order by IDC,IDJug,IDG,IDCN,carr,serial");

	//echo "SELECT _tjugadahi.*,_tconsecionario.IDG  FROM _tjugadahi,_tconsecionario  where _tjugadahi.IDC=_tconsecionario.IDC    and  _tconsecionario.estatus=1 and  anulado=0  ".$add." order by IDJug,IDG,IDC,IDCN,carr,serial" ;
