...
$result = mysqli_query($GLOBALS['link'], "SELECT * FROM _tbaccesoreportes  ");
$row = mysqli_fetch_array($result);


$add = '';

if ($row['AccesoaReportes'] == 1) :
	...
	$add2 = "Select IDJ From _jornadabb Where STR_TO_DATE(Fecha,'%d/%m/%Y') BETWEEN STR_TO_DATE('" . $d1 . "','%d/%m/%Y') and STR_TO_DATE('" . $d2 . "','%d/%m/%Y')";
	...
	while ($row = mysqli_fetch_array($result)) {
		...
		$result2 = mysqli_query($GLOBALS['link'], "SELECT sum(ventas) as ven, sum(premios) as pre FROM totales  where   IDC='" . $row['IDC'] . "' and tipodejugada=1  " . $add);
		$row2 = mysqli_fetch_array($result2);
		$totalventas1 += $row2['ven'];
		$totalpremio1 += $row2['pre'];
		$result2 = mysqli_query($GLOBALS['link'], "SELECT  sum(ventas) as ven, sum(premios) as  pre FROM totales  where   IDC='" . $row['IDC'] . "' and tipodejugada=2  " . $add);
		$row2 = mysqli_fetch_array($result2);
		$totalventas += $row2['ven'];
		$totalpremio += $row2['pre'];
		$result2 = mysqli_query($GLOBALS['link'], "SELECT  sum(ventas) as ven, sum(premios) as  pre FROM totales  where   IDC='" . $row['IDC'] . "' and tipodejugada=3  " . $add);
		$row2 = mysqli_fetch_array($result2);
		$totalventas3 += $row2['ven'];
		$totalpremio3 += $row2['pre'];
		...
		if ($totalventas1 != 0 || $totalventas != 0 || $totalventas3 != 0) :
		...
		$aa[3] = number_format($totalventas3, 2, '.', '');

		if ($pdv == -3) :
			$aa[4] = number_format($mm, 2, '.', '');
		else :
			$aa[4] = number_format(($totalventas3 * $pdv3) / 100, 2, '.', '');
		endif;


		$aa[5] = number_format($totalpremio3, 2, '.', '');

		$aa[6] = number_format(($totalventas3 - $aa[4]) - $totalpremio3, 2, '.', '');

		if ((($totalventas3 - $aa[4]) - $totalpremio3) < 0) :
			$pr = $pdp1;
		else :
			$pr = $pdp;
		endif;
	...
	echo "En estos Momentos No Podemos Dar Esta Informacion...";
