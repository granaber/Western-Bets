...
if ($pfc == 0) :
  ...
  $result = mysqli_query($GLOBALS['link'], "SELECT * FROM _tgrupo Order by IDG DESC ");
  ...
  $result = mysqli_query($GLOBALS['link'], "SELECT * FROM _tgrupo where IDG=" . $pfc);
  $row = mysqli_fetch_array($result);
  $idg = $row["IDG"];
  $nm = $row["Descrip"];
  $res = $row["Responsable"];
  ...
  $porce_Parlay = $row["porce_parlay"];
  $porce_Derecho = $row["porce_derecho"];
endif;

?>
<div id="fromGrupo" style="background:#BAC6D8;width:415px; height:1000px">
        ...
        <td width="479">
          <table border="0" cellpadding="2" cellspacing="1" width="100%">
            ...
            <tbody>
              ...
              <tr title="">
                <td width="36%"><strong>Codigo</strong></td>
                <td width="64%" id="n_idg" title="<?php echo $idg; ?>"><span style="font-size:14px; color:#FF9900"><strong><?php echo $idg; ?></strong></span><br /></td>
              </tr>
              ...
              <tr>
                <td>Telefono</td>
                <td><input name="text3" type="text" id="telefono" onChange="validar4(event);" onkeyup="pulsart(event,'direccion'); validar4(event);" value='<?php echo $tlf; ?>' />
                  <font color="red">*</font>
                  <img id="imgtelefono" src="media/serro.png" height="16" width="16" style="display:none" title="" />
                </td>
              </tr>

              <tr>
                <td>Direccion</td>
                <td><input name="text2" type="text" id="direccion" onChange="validar4(event);" onkeyup="pulsart(event,'direccion'); validar4(event);" value='<?php echo $dr; ?>' />
                  <font color="red">*</font>
                  <img id="imgdireccion" src="media/serro.png" height="16" width="16" style="display:none" title="" />
                </td>
              </tr>
              <tr style="background:#666; color:#FFF">
                <td>Participacion</td>
                <td><input name="" id='Participacion' type="text" size="6" value="<? echo $participacion; ?>">
                  %</td>
              </tr>
              <tr style="background:#666; color:#FFF">
                <td>% de Ventas Derecho</td>
                <td><input name="" id='porce_Derecho' type="text" size="6" value="<? echo $porce_Derecho; ?>">
                  %</td>
              </tr>
              <tr style="background:#666; color:#FFF">
                <td>% de Ventas Parlay</td>
                <td><input name="" id='porce_Parlay' type="text" size="6" value="<? echo $porce_Parlay; ?>">
                  %</td>
              </tr>
              <tr>
                <td>Pertenece a la Banca</td>
                <td><label>
                    <select name="select2" id="IDB">
                      <?
                      $sql = $accesoBanca == 0 ? "SELECT * FROM _tbanca Order by IDB " : "SELECT * FROM _tbanca where IDB=$accesoBanca";
...
<script>
  ...
  w1 = dhxWins1.createWindow("w1", 350, 350, 350, 300);
  w1.setText('Grupo(Intermediarios)');
  w1.attachObject('fromGrupo');
