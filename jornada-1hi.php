...
if ($opc == -1) :
  ...
  $result = mysqli_query($GLOBALS['link'], "SELECT * FROM _tconfjornadahi order by IDCN DESC ");
  ...
  $result = mysqli_query($GLOBALS['link'], "SELECT * FROM _tconfjornadahi where IDCN=" . $opc);
  $row = mysqli_fetch_array($result);
  $numero = $row["IDCN"];
  $idhipo = $row["IDhipo"];
  $NTDP4 = $row["NTDP4"];
...
?>
<p style="color:#FFCC00; font-size:16px">Parametro de Jornada</p>

<table border="0" cellpadding="3" cellspacing="0" width="805">
  <tbody>
    <tr>
      <td><strong style="color:#FFFFFF; font-size:12px">Jornada No.: <span id="nj" title="<?php echo $numero; ?>"><?php echo $numero; ?></span></strong></p>
      </td>
      <td><strong style="color:#FFFFFF; font-size:12px">Indique la Fecha:</strong>
        <input type="text" name="fecha_2" id="fecha_2" size="10" maxlength="10" value="<?php echo $fc; ?>" onFocus="cargarcampos2();">
      </td>
      <td><strong style="color:#FFFFFF; font-size:12px">Hipodromo </strong>
    ...
    <tr>
      <td><strong style="color:#FFFFFF; font-size:12px">Número de Carreras</strong></td>
      <td><select id="cmbCarreras" size="1">
      ...
      <td><strong style="color:#FFFFFF; font-size:12px">Estatus:</strong>
        <select id="testatus">
    ...
    <tr>
      <td><strong style="color:#FFFFFF; font-size:12px">Número de Tandas de Pick Four</strong></td>
      <!-- Cargamos los select correspondientes a cada juego que posea tandas  OnChange="cambiarFoco(document.forms[1].elements[])-->
      <td colspan="2"><select id="cmbTandas_1" name="cmbTandas_1" size="1">
    ...
    <tr>
      <td><strong style="color:#FFFFFF; font-size:12px">Número de Tandas de Pick Tree</strong></td>
      <!-- Cargamos los select correspondientes a cada juego que posea tandas  OnChange="cambiarFoco(document.forms[1].elements[])-->
      <td colspan="2"><select id="cmbTandas_2" name="cmbTandas_2" size="1">
    ...
    <tr>
      <td><strong style="color:#FFFFFF; font-size:12px">Número de Tandas de Daily Double</strong></td>
      <!-- Cargamos los select correspondientes a cada juego que posea tandas  OnChange="cambiarFoco(document.forms[1].elements[])-->
      <td colspan="2"><select id="cmbTandas_3" name="cmbTandas_3" size="1">
...
<p class="Estilo39">
  <input type="submit" name="Submit" value="Actualizar Tabla" onclick="makeResEsphi('jornada-2hi.php','tabladeconfiguracion')" /> <input type="submit" name="Submit" value="&lt;-Volver" onclick="javascript:makeRequest('jornadahi.php');" />
</p>