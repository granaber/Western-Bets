...
//|1-2-|3-
function verretirados($ListaApuesta, $ListaRetirados, $cantidadMax)
{
	...
	for ($j = 0; $j <= count($ListaApuesta) - 1; $j++) {
		...
		if (intval($ListaApuesta[$j]) != 0) :
			if (contarvalor($ListaRetirados, $ListaApuesta[$j]) == 0) :
				...
				if (count($ListaApuesta) > 2) :
					if ((intval($ListaApuesta[$j]) + 1) > $cantidadMax) :
						$listaNApuesta[] = 1;
					else :
						$listaNApuesta[] = intval($ListaApuesta[$j]) + 1;
					endif;
				endif;
				$resultados[0] = true;
			endif;
...
function escrute_porpuesto($Jugada, $IDCN, $carrera, $IDJug, $pagado, $valorReal, $serial, $Vestatus)
{
	...
	for ($i = 1; $i <= count($Jugada_x_Carrera) - 1; $i++) {
		...
		$resultado = verretirados($ejemplares_apostados, $ejemplares_retirados, $aCanti[$i - 1]);
		if ($resultado[0]) :
			// print_r( $resultado);
			$Vrecalculo = true;
