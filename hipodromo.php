...
if ($pfc == 0) :
  ...
  $result = mysqli_query($GLOBALS['link'], "SELECT * FROM _hipodromos Order by _idhipo DESC ");
  ...
  $result = mysqli_query($GLOBALS['link'], "SELECT * FROM _hipodromos where _idhipo=" . $pfc);
  $row = mysqli_fetch_array($result);
  $idh = $row["_idhipo"];
  $nm = $row["Descripcion"];
  $sig = $row["siglas"];
    ...
    <tbody>
      <tr>
        <td width="362">
          ...
          <p>
            <?php if (!$pfc == 0) :
              echo  "<input id='btnguardar'  value='Modificar' type='submit'  onclick='grabar_hipo();'/>";
              echo  "<input id='btneliminar'  value='Eliminar Hipodromo' type='submit'  onclick='elimnar_hipo();'/>";
            else :
              echo  "<input id='btnguardar'  value='Guardar Nuevo Hipodromo' type='submit'  disabled='disabled' onclick='grabar_hipo();'/>";
            endif;
            ?>
